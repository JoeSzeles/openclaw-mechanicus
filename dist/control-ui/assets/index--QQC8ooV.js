...
`)},Ip=(e,t,n)=>{const s=e.trim();if(!s)return[];const i=s.length?s.split(/\s+/):[],a=i.length?i[i.length-1]:"",[o,r]=a.includes(":")?[a.slice(0,a.indexOf(":")),a.slice(a.indexOf(":")+1)]:["",""],c=o.toLowerCase(),d=r.toLowerCase(),g=C=>{const k=new Set;for(const A of C)A&&k.add(A);return Array.from(k)},p=g(t.map(C=>C.agentId)).slice(0,6),h=g(t.map(C=>C.channel)).slice(0,6),u=g([...t.map(C=>C.modelProvider),...t.map(C=>C.providerOverride),...n?.byProvider.map(C=>C.provider)??[]]).slice(0,6),f=g([...t.map(C=>C.model),...n?.byModel.map(C=>C.model)??[]]).slice(0,6),m=g(n?.tools.tools.map(C=>C.name)??[]).slice(0,6);if(!c)return[{label:"agent:",value:"agent:"},{label:"channel:",value:"channel:"},{label:"provider:",value:"provider:"},{label:"model:",value:"model:"},{label:"tool:",value:"tool:"},{label:"has:errors",value:"has:errors"},{label:"has:tools",value:"has:tools"},{label:"minTokens:",value:"minTokens:"},{label:"maxCost:",value:"maxCost:"}];const w=[],S=(C,k)=>{for(const A of k)(!d||A.toLowerCase().includes(d))&&w.push({label:`${C}:${A}`,value:`${C}:${A}`})};switch(c){case"agent":S("agent",p);break;case"channel":S("channel",h);break;case"provider":S("provider",u);break;case"model":S("model",f);break;case"tool":S("tool",m);break;case"has":["errors","tools","context","usage","model","provider"].forEach(C=>{(!d||C.includes(d))&&w.push({label:`has:${C}`,value:`has:${C}`})});break}return w},Rp=(e,t)=>{const n=e.trim();if(!n)return`${t} `;const s=n.split(/\s+/);return s[s.length-1]=t,`${s.join(" ")} `},Lt=e=>e.trim().toLowerCase(),Pp=(e,t)=>{const n=e.trim();if(!n)return`${t} `;const s=n.split(/\s+/),i=s[s.length-1]??"",a=t.includes(":")?t.split(":")[0]:null,o=i.includes(":")?i.split(":")[0]:null;return i.endsWith(":")&&a&&o===a?(s[s.length-1]=t,`${s.join(" ")} `):s.includes(t)?`${s.join(" ")} `:`${s.join(" ")} ${t} `},Ro=(e,t)=>{const s=e.trim().split(/\s+/).filter(Boolean).filter(i=>i!==t);return s.length?`${s.join(" ")} `:""},Po=(e,t,n)=>{const s=Lt(t),a=[...ha(e).filter(o=>Lt(o.key??"")!==s).map(o=>o.raw),...n.map(o=>`${t}:${o}`)];return a.length?`${a.join(" ")} `:""};function gt(e,t){return t===0?0:e/t*100}function Dp(e){const t=e.totalCost||0;return{input:{tokens:e.input,cost:e.inputCost||0,pct:gt(e.inputCost||0,t)},output:{tokens:e.output,cost:e.outputCost||0,pct:gt(e.outputCost||0,t)},cacheRead:{tokens:e.cacheRead,cost:e.cacheReadCost||0,pct:gt(e.cacheReadCost||0,t)},cacheWrite:{tokens:e.cacheWrite,cost:e.cacheWriteCost||0,pct:gt(e.cacheWriteCost||0,t)},totalCost:t}}function Np(e,t,n,s,i,a,o,r){if(!(e.length>0||t.length>0||n.length>0))return v;const d=n.length===1?s.find(f=>f.key===n[0]):null,g=d?(d.label||d.key).slice(0,20)+((d.label||d.key).length>20?"…":""):n.length===1?n[0].slice(0,8)+"…":`${n.length} sessions`,p=d?d.label||d.key:n.length===1?n[0]:n.join(", "),h=e.length===1?e[0]:`${e.length} days`,u=t.length===1?`${t[0]}:00`:`${t.length} hours`;return l`
  ...
  `}const Up=.75,Hp=8,jp=.06,Wn=5,Le=12,ct=.7;function pt(e,t){return!t||t<=0?0:e/t*100}function Kp(){return v}function Bl(e){return e<1e12?e*1e3:e}function Wp(e,t,n){const s=Math.min(t,n),i=Math.max(t,n);return e.filter(a=>{if(a.timestamp<=0)return!0;const o=Bl(a.timestamp);return o>=s&&o<=i})}function qp(e,t,n){const s=t||e.usage;if(!s)return l`
    ...
    <div class="card session-detail-panel">
      ...
      <div class="session-detail-content">
        ...
        <div class="session-detail-bottom">
          ${Zp(u,f,m,w,S,C,k,A,E,T,ne?r:null,ne?c:null)}
    ...
    <div class="session-timeseries-compact">
      ...
      <div class="timeseries-chart-wrapper" style="position: relative; cursor: crosshair;">
        ...
        >
          ...
          <!-- Y axis labels -->
          <text x="${_.left-4}" y="${_.top+5}" text-anchor="end" class="ts-axis-label">${B(N)}</text>
          ...
          `:v}
          <!-- Bars -->
          ${h.map((U,ie)=>{const le=D[ie],fe=_.left+ie*(ce+X),Te=le/N*Q,Ke=_.top+Q-Te,he=[new Date(U.timestamp).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),`${B(le)} tokens`];pe&&(he.push(`Out ${B(U.output)}`),he.push(`In ${B(U.input)}`),he.push(`CW ${B(U.cacheWrite)}`),he.push(`CR ${B(U.cacheRead)}`));const Oe=he.join(" · "),We=k&&(ie<T||ie>=I);if(!pe)return kt`<rect x="${fe}" y="${Ke}" width="${ce}" height="${Te}" class="ts-bar${We?" dimmed":""}" rx="1"><title>${Oe}</title></rect>`;const qe=[{value:U.output,cls:"output"},{value:U.input,cls:"input"},{value:U.cacheWrite,cls:"cache-write"},{value:U.cacheRead,cls:"cache-read"}];let Ve=_.top+Q;const at=We?" dimmed":"";return kt`
              ${qe.map(ot=>{if(ot.value<=0||le<=0)return v;const xt=Te*(ot.value/le);return Ve-=xt,kt`<rect x="${fe}" y="${Ve}" width="${ce}" height="${xt}" class="ts-bar ${ot.cls}${at}" rx="1"><title>${Oe}</title></rect>`})}
            `})}
        ...
        ${(()=>{const U=`${(se/ne*100).toFixed(1)}%`,ie=`${(V/ne*100).toFixed(1)}%`,le=fe=>Te=>{if(!p)return;Te.preventDefault(),Te.stopPropagation();const it=Te.currentTarget.closest(".timeseries-chart-wrapper")?.querySelector("svg");if(!it)return;const he=it.getBoundingClientRect(),Oe=he.width,We=_.left/ne*Oe,Ve=(ne-_.right)/ne*Oe-We,at=Be=>{const Se=Math.max(0,Math.min(1,(Be-he.left-We)/Ve));return Math.min(Math.floor(Se*h.length),h.length-1)},ot=fe==="left"?se:V,xt=he.left+ot/ne*Oe,Fs=Te.clientX-xt;document.body.style.cursor="col-resize";const Kt=Be=>{const Se=Be.clientX-Fs,an=at(Se),Wt=h[an];if(Wt)if(fe==="left"){const lt=g??h[h.length-1].timestamp;p(Math.min(Wt.timestamp,lt),lt)}else{const lt=d??h[0].timestamp;p(lt,Math.max(Wt.timestamp,lt))}},rt=()=>{document.body.style.cursor="",document.removeEventListener("mousemove",Kt),document.removeEventListener("mouseup",rt)};document.addEventListener("mousemove",Kt),document.addEventListener("mouseup",rt)};return l`
...
`);function nf(e){if(e.loading&&!e.totals)return l`
    ...
    `;const t=e.chartMode==="tokens",n=e.query.trim().length>0,s=e.queryDraft.trim().length>0,i=[...e.sessions].toSorted((M,D)=>{const N=t?M.usage?.totalTokens??0:M.usage?.totalCost??0;return(t?D.usage?.totalTokens??0:D.usage?.totalCost??0)-N}),a=e.selectedDays.length>0?i.filter(M=>{if(M.usage?.activityDates?.length)return M.usage.activityDates.some(K=>e.selectedDays.includes(K));if(!M.updatedAt)return!1;const D=new Date(M.updatedAt),N=`${D.getFullYear()}-${String(D.getMonth()+1).padStart(2,"0")}-${String(D.getDate()).padStart(2,"0")}`;return e.selectedDays.includes(N)}):i,o=(M,D)=>{if(D.length===0)return!0;const N=M.usage,K=N?.firstActivity??M.updatedAt,ce=N?.lastActivity??M.updatedAt;if(!K||!ce)return!1;const X=Math.min(K,ce),se=Math.max(K,ce);let V=X;for(;V<=se;){const U=new Date(V),ie=va(U,e.timeZone);if(D.includes(ie))return!0;const le=ma(U,e.timeZone);V=Math.min(le.getTime(),se)+1}return!1},r=e.selectedHours.length>0?a.filter(M=>o(M,e.selectedHours)):a,c=mp(r,e.query),d=c.sessions,g=c.warnings,p=Ip(e.queryDraft,i,e.aggregates),h=ha(e.query),u=M=>{const D=Lt(M);return h.filter(N=>Lt(N.key??"")===D).map(N=>N.value).filter(Boolean)},f=M=>{const D=new Set;for(const N of M)N&&D.add(N);return Array.from(D)},m=f(i.map(M=>M.agentId)).slice(0,12),w=f(i.map(M=>M.channel)).slice(0,12),S=f([...i.map(M=>M.modelProvider),...i.map(M=>M.providerOverride),...e.aggregates?.byProvider.map(M=>M.provider)??[]]).slice(0,12),C=f([...i.map(M=>M.model),...e.aggregates?.byModel.map(M=>M.model)??[]]).slice(0,12),k=f(e.aggregates?.tools.tools.map(M=>M.name)??[]).slice(0,12),A=e.selectedSessions.length===1?e.sessions.find(M=>M.key===e.selectedSessions[0])??d.find(M=>M.key===e.selectedSessions[0]):null,E=M=>M.reduce((D,N)=>(N.usage&&(D.input+=N.usage.input,D.output+=N.usage.output,D.cacheRead+=N.usage.cacheRead,D.cacheWrite+=N.usage.cacheWrite,D.totalTokens+=N.usage.totalTokens,D.totalCost+=N.usage.totalCost,D.inputCost+=N.usage.inputCost??0,D.outputCost+=N.usage.outputCost??0,D.cacheReadCost+=N.usage.cacheReadCost??0,D.cacheWriteCost+=N.usage.cacheWriteCost??0,D.missingCostEntries+=N.usage.missingCostEntries??0),D),{input:0,output:0,cacheRead:0,cacheWrite:0,totalTokens:0,totalCost:0,inputCost:0,outputCost:0,cacheReadCost:0,cacheWriteCost:0,missingCostEntries:0}),T=M=>e.costDaily.filter(N=>M.includes(N.date)).reduce((N,K)=>(N.input+=K.input,N.output+=K.output,N.cacheRead+=K.cacheRead,N.cacheWrite+=K.cacheWrite,N.totalTokens+=K.totalTokens,N.totalCost+=K.totalCost,N.inputCost+=K.inputCost??0,N.outputCost+=K.outputCost??0,N.cacheReadCost+=K.cacheReadCost??0,N.cacheWriteCost+=K.cacheWriteCost??0,N),{input:0,output:0,cacheRead:0,cacheWrite:0,totalTokens:0,totalCost:0,inputCost:0,outputCost:0,cacheReadCost:0,cacheWriteCost:0,missingCostEntries:0});let I,H;const Z=i.length;if(e.selectedSessions.length>0){const M=d.filter(D=>e.selectedSessions.includes(D.key));I=E(M),H=M.length}else e.selectedDays.length>0&&e.selectedHours.length===0?(I=T(e.selectedDays),H=d.length):e.selectedHours.length>0||n?(I=E(d),H=d.length):(I=e.totals,H=Z);const te=e.selectedSessions.length>0?d.filter(M=>e.selectedSessions.includes(M.key)):n||e.selectedHours.length>0?d:e.selectedDays.length>0?a:i,R=Tp(te,e.aggregates),q=e.selectedSessions.length>0?(()=>{const M=d.filter(N=>e.selectedSessions.includes(N.key)),D=new Set;for(const N of M)for(const K of N.usage?.activityDates??[])D.add(K);return D.size>0?e.costDaily.filter(N=>D.has(N.date)):e.costDaily})():e.costDaily,ne=_p(te,I,R),oe=!e.loading&&!e.totals&&e.sessions.length===0,_=(I?.missingCostEntries??0)>0||(I?I.totalTokens>0&&I.totalCost===0&&I.input+I.output+I.cacheRead+I.cacheWrite>0:!1),j=[{label:"Today",days:1},{label:"7d",days:7},{label:"30d",days:30}],Q=M=>{const D=new Date,N=new Date;N.setDate(N.getDate()-(M-1)),e.onStartDateChange(ti(N)),e.onEndDateChange(ti(D))},re=(M,D,N)=>{if(N.length===0)return v;const K=u(M),ce=new Set(K.map(V=>Lt(V))),X=N.length>0&&N.every(V=>ce.has(Lt(V))),se=K.length;return l`
      <details
      ...
      >
        ...
        <div class="usage-filter-popover">
          <div class="usage-filter-actions">
            <button
              class="btn btn-sm"
              @click=${V=>{V.preventDefault(),V.stopPropagation(),e.onQueryDraftChange(Po(e.queryDraft,M,N))}}
              ?disabled=${X}
            >
              Select All
            </button>
          ...
          <div class="usage-filter-options">
            ${N.map(V=>{const U=ce.has(Lt(V));return l`
    ...
    <section class="card usage-header ${e.headerPinned?"pinned":""}">
      <div class="usage-header-row">
        ...
        <div class="usage-header-metrics">
          ...
          <details
            ...
            @toggle=${M=>{const D=M.currentTarget;if(!D.open)return;const N=K=>{K.composedPath().includes(D)||(D.open=!1,window.removeEventListener("click",N,!0))};window.addEventListener("click",N,!0)}}
          >
      ...
      <div class="usage-header-row">
        <div class="usage-controls">
          ${Np(e.selectedDays,e.selectedHours,e.selectedSessions,e.sessions,e.onClearDays,e.onClearHours,e.onClearSessions,e.onClearFilters)}
    ...
    <div class="chat-controls">
      <label class="field chat-controls__session">
        <select
          ...
          @change=${d=>{const g=d.target.value;e.sessionKey=g,e.chatMessage="",e.chatStream=null,e.chatStreamStartedAt=null,e.chatRunId=null,e.resetToolStream(),e.resetChatScroll(),e.applySettings({...e.settings,sessionKey:g,lastActiveSessionKey:g}),e.loadAssistantIdentity(),ug(e,g),Dt(e)}}
        >
  ...
  `}function sh(e){const t=Rs(e.configForm,e.agentId),n=t.entry?.tools??{},s=t.globalTools??{},i=n.profile??s.profile??"full",a=n.profile?"agent override":s.profile?"global default":"default",o=Array.isArray(n.allow)&&n.allow.length>0,r=Array.isArray(s.allow)&&s.allow.length>0,c=!!e.configForm&&!e.configLoading&&!e.configSaving&&!o,d=o?[]:Array.isArray(n.alsoAllow)?n.alsoAllow:[],g=o?[]:Array.isArray(n.deny)?n.deny:[],p=o?{allow:n.allow??[],deny:n.deny??[]}:Wf(i)??void 0,h=Uo.flatMap(S=>S.tools.map(C=>C.id)),u=S=>{const C=Kf(S,p),k=Wo(S,d),A=Wo(S,g);return{allowed:(C||k)&&!A,baseAllowed:C,denied:A}},f=h.filter(S=>u(S).allowed).length,m=(S,C)=>{const k=new Set(d.map(I=>je(I)).filter(I=>I.length>0)),A=new Set(g.map(I=>je(I)).filter(I=>I.length>0)),E=u(S).baseAllowed,T=je(S);C?(A.delete(T),E||k.add(T)):(k.delete(T),A.add(T)),e.onOverridesChange(e.agentId,[...k],[...A])},w=S=>{const C=new Set(d.map(A=>je(A)).filter(A=>A.length>0)),k=new Set(g.map(A=>je(A)).filter(A=>A.length>0));for(const A of h){const E=u(A).baseAllowed,T=je(A);S?(k.delete(T),E||C.add(T)):(C.delete(T),k.add(T))}e.onOverridesChange(e.agentId,[...C],[...k])};return l`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
  ...
  `}function ih(e){const t=!!e.configForm&&!e.configLoading&&!e.configSaving,n=Rs(e.configForm,e.agentId),s=Array.isArray(n.entry?.skills)?n.entry?.skills:void 0,i=new Set((s??[]).map(u=>u.trim()).filter(Boolean)),a=s!==void 0,o=!!(e.report&&e.activeAgentId===e.agentId),r=o?e.report?.skills??[]:[],c=e.filter.trim().toLowerCase(),d=c?r.filter(u=>[u.name,u.description,u.source].join(" ").toLowerCase().includes(c)):r,g=Wl(d),p=a?r.filter(u=>i.has(u.name)).length:r.length,h=r.length;return l`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
    ...
    <div class="agents-layout">
      <section class="card agents-sidebar">
        ...
        <div class="agent-list" style="margin-top: 12px;">
          ${t.length===0?l`
                ...
                `:t.map(a=>{const o=jl(a.id,n),r=Is(a,e.agentIdentityById[a.id]??null);return l`
                    <button
                      type="button"
      ...
      <section class="agents-main">
        ${i?l`
                ...
                ${e.activePanel==="tools"?sh({agentId:i.id,configForm:e.configForm,configLoading:e.configLoading,configSaving:e.configSaving,configDirty:e.configDirty,onProfileChange:e.onToolsProfileChange,onOverridesChange:e.onToolsOverridesChange,onConfigReload:e.onConfigReload,onConfigSave:e.onConfigSave}):v}
                ...
                ${e.activePanel==="channels"?Jf({context:Ho(i,e.configForm,e.agentFilesList,n,e.agentIdentityById[i.id]??null),configForm:e.configForm,snapshot:e.channelsSnapshot,loading:e.channelsLoading,error:e.channelsError,lastSuccess:e.channelsLastSuccess,onRefresh:e.onChannelsRefresh}):v}
                ${e.activePanel==="cron"?eh({context:Ho(i,e.configForm,e.agentFilesList,n,e.agentIdentityById[i.id]??null),agentId:i.id,jobs:e.cronJobs,status:e.cronStatus,loading:e.cronLoading,error:e.cronError,onRefresh:e.onCronRefresh}):v}
              `:l`
                <div class="card">
  ...
  `}function lh(e,t,n){const s=jl(e.id,t),i=_i(e),a=e.identity?.theme?.trim()||"Agent workspace and routing.",o=Is(e,n);return l`
  ...
  `}function dh(e){const{agent:t,configForm:n,agentFilesList:s,agentIdentity:i,agentIdentityLoading:a,agentIdentityError:o,configLoading:r,configSaving:c,configDirty:d,onConfigReload:g,onConfigSave:p,onModelChange:h,onModelFallbacksChange:u}=e,f=Rs(n,t.id),w=(s&&s.agentId===t.id?s.workspace:null)||f.entry?.workspace||f.defaults?.workspace||"default",S=f.entry?.model?xn(f.entry?.model):xn(f.defaults?.model),C=xn(f.defaults?.model),k=Ko(f.entry?.model)||(S!=="-"?jo(S):null),A=Ko(f.defaults?.model)||(C!=="-"?jo(C):null),E=k??A??null,T=Bf(f.entry?.model),I=T?T.join(", "):"",H=i?.name?.trim()||t.identity?.name?.trim()||t.name?.trim()||f.entry?.name||"-",te=Is(t,i)||"-",R=Array.isArray(f.entry?.skills)?f.entry?.skills:null,q=R?.length??null,ne=a?"Loading…":o?"Unavailable":"",oe=!!(e.defaultId&&t.id===e.defaultId);return l`
    <section class="card">
  ...
  `};function mt(e){const{schema:t,value:n,path:s,hints:i,unsupported:a,disabled:o,onPatch:r}=e,c=e.showLabel??!0,d=Ee(t),g=Ie(s,i),p=g?.label??t.title??nt(String(s.at(-1))),h=g?.help??t.description,u=Gi(s);if(a.has(u))return l`<div class="cfg-field cfg-field--error">
      <div class="cfg-field__label">${p}</div>
      <div class="cfg-field__error">Unsupported schema node. Use Raw mode.</div>
    </div>`;if(t.anyOf||t.oneOf){const m=(t.anyOf??t.oneOf??[]).filter(E=>!(E.type==="null"||Array.isArray(E.type)&&E.type.includes("null")));if(m.length===1)return mt({...e,schema:m[0]});const w=E=>{if(E.const!==void 0)return E.const;if(E.enum&&E.enum.length===1)return E.enum[0]},S=m.map(w),C=S.every(E=>E!==void 0);if(C&&S.length>0&&S.length<=5){const E=n??t.default;return l`
        <div class="cfg-field">
      ...
      `}return Vo({...e,options:f,value:n??t.default})}if(d==="object")return hh(e);if(d==="array")return vh(e);if(d==="boolean"){const f=typeof n=="boolean"?n:typeof t.default=="boolean"?t.default:!1;return l`
  ...
  `}function Vo(e){const{schema:t,value:n,path:s,hints:i,disabled:a,options:o,onPatch:r}=e,c=e.showLabel??!0,d=Ie(s,i),g=d?.label??t.title??nt(String(s.at(-1))),p=d?.help??t.description,h=n??t.default,u=o.findIndex(m=>m===h||String(m)===String(h)),f="__unset__";return l`
    <div class="cfg-field">
      ${c?l`<label class="cfg-field__label">${g}</label>`:v}
      ${p?l`<div class="cfg-field__help">${p}</div>`:v}
      <select
  ...
  `}function hh(e){const{schema:t,value:n,path:s,hints:i,unsupported:a,disabled:o,onPatch:r}=e,c=Ie(s,i),d=c?.label??t.title??nt(String(s.at(-1))),g=c?.help??t.description,p=n??t.default,h=p&&typeof p=="object"&&!Array.isArray(p)?p:{},u=t.properties??{},m=Object.entries(u).toSorted((A,E)=>{const T=Ie([...s,A[0]],i)?.order??0,I=Ie([...s,E[0]],i)?.order??0;return T!==I?T-I:A[0].localeCompare(E[0])}),w=new Set(Object.keys(u)),S=t.additionalProperties,C=!!S&&typeof S=="object",k=l`
    ${m.map(([A,E])=>mt({schema:E,value:h[A],path:[...s,A],hints:i,unsupported:a,disabled:o,onPatch:r}))}
    ${C?mh({schema:S,value:h,path:s,hints:i,unsupported:a,disabled:o,reservedKeys:w,onPatch:r}):v}
  `;return s.length===1?l`
  ...
  `}function vh(e){const{schema:t,value:n,path:s,hints:i,unsupported:a,disabled:o,onPatch:r}=e,c=e.showLabel??!0,d=Ie(s,i),g=d?.label??t.title??nt(String(s.at(-1))),p=d?.help??t.description,h=Array.isArray(t.items)?t.items[0]:t.items;if(!h)return l`
      <div class="cfg-field cfg-field--error">
          ...
          ${u.map((f,m)=>l`
            <div class="cfg-array__item">
              ...
              <div class="cfg-array__item-content">
                ${mt({schema:h,value:f,path:[...s,m],hints:i,unsupported:a,disabled:o,showLabel:!1,onPatch:r})}
              </div>
            </div>
          `)}
        </div>
  ...
  `},wa={env:{label:"Environment Variables",description:"Environment variables passed to the gateway process"},update:{label:"Updates",description:"Auto-update settings and release channel"},agents:{label:"Agents",description:"Agent configurations, models, and identities"},auth:{label:"Authentication",description:"API keys and authentication profiles"},channels:{label:"Channels",description:"Messaging channels (Telegram, Discord, Slack, etc.)"},messages:{label:"Messages",description:"Message handling and routing settings"},commands:{label:"Commands",description:"Custom slash commands"},hooks:{label:"Hooks",description:"Webhooks and event hooks"},skills:{label:"Skills",description:"Skill packs and capabilities"},tools:{label:"Tools",description:"Tool configurations (browser, search, etc.)"},gateway:{label:"Gateway",description:"Gateway server settings (port, auth, binding)"},wizard:{label:"Setup Wizard",description:"Setup wizard state and history"},meta:{label:"Metadata",description:"Gateway metadata and version information"},logging:{label:"Logging",description:"Log levels and output configuration"},browser:{label:"Browser",description:"Browser automation settings"},ui:{label:"UI",description:"User interface preferences"},models:{label:"Models",description:"AI model configurations and providers"},bindings:{label:"Bindings",description:"Key bindings and shortcuts"},broadcast:{label:"Broadcast",description:"Broadcast and notification settings"},audio:{label:"Audio",description:"Audio input/output settings"},session:{label:"Session",description:"Session management and persistence"},cron:{label:"Cron",description:"Scheduled tasks and automation"},web:{label:"Web",description:"Web server and API settings"},discovery:{label:"Discovery",description:"Service discovery and networking"},canvasHost:{label:"Canvas Host",description:"Canvas rendering and display"},talk:{label:"Talk",description:"Voice and speech settings"},plugins:{label:"Plugins",description:"Plugin management and extensions"}};function Qo(e){return Go[e]??Go.default}function bh(e,t,n){if(!n)return!0;const s=n.toLowerCase(),i=wa[e];return e.toLowerCase().includes(s)||i&&(i.label.toLowerCase().includes(s)||i.description.toLowerCase().includes(s))?!0:vn(t,s)}function vn(e,t){if(e.title?.toLowerCase().includes(t)||e.description?.toLowerCase().includes(t)||e.enum?.some(s=>String(s).toLowerCase().includes(t)))return!0;if(e.properties){for(const[s,i]of Object.entries(e.properties))if(s.toLowerCase().includes(t)||vn(i,t))return!0}if(e.items){const s=Array.isArray(e.items)?e.items:[e.items];for(const i of s)if(i&&vn(i,t))return!0}if(e.additionalProperties&&typeof e.additionalProperties=="object"&&vn(e.additionalProperties,t))return!0;const n=e.anyOf??e.oneOf??e.allOf;if(n){for(const s of n)if(s&&vn(s,t))return!0}return!1}function yh(e){if(!e.schema)return l`
    ...
    <div class="config-form config-form--modern">
      ${g?(()=>{const{sectionKey:p,subsectionKey:h,schema:u}=g,f=Ie([p,h],e.uiHints),m=f?.label??u.title??nt(h),w=f?.help??u.description??"",S=n[p],C=S&&typeof S=="object"?S[h]:void 0,k=`config-section-${p}-${h}`;return l`
  ...
  `}function Yo(e){return e?e.length<=20?e:`${e.slice(0,8)}...${e.slice(-8)}`:"n/a"}function Ih(e){const{props:t,nostr:n,nostrAccounts:s,accountCountLabel:i,profileFormState:a,profileFormCallbacks:o,onEditProfile:r}=e,c=s[0],d=n?.configured??c?.configured??!1,g=n?.running??c?.running??!1,p=n?.publicKey??c?.publicKey,h=n?.lastStartAt??c?.lastStartAt??null,u=n?.lastError??c?.lastError??null,f=s.length>1,m=a!=null,w=C=>{const k=C.publicKey,A=C.profile,E=A?.displayName??A?.name??C.name??C.accountId;return l`
    ...
    `},S=()=>{if(m&&o)return xd({state:a,callbacks:o,accountId:s[0]?.accountId??"default"});const C=c?.profile??n?.profile,{name:k,displayName:A,about:E,picture:T,nip05:I}=C??{},H=k||A||E||T||I;return l`
  ...
  `}function Rh(e,t){const n=t.snapshot,s=n?.channels;if(!n||!s)return!1;const i=s[e],a=typeof i?.configured=="boolean"&&i.configured,o=typeof i?.running=="boolean"&&i.running,r=typeof i?.connected=="boolean"&&i.connected,d=(n.channelAccounts?.[e]??[]).some(g=>g.configured||g.running||g.connected);return a||o||r||d}function Ph(e,t){return t?.[e]?.length??0}function Zl(e,t){const n=Ph(e,t);return n<2?v:l`<div class="account-count">Accounts (${n})</div>`}function Dh(e){const{props:t,signal:n,accountCountLabel:s}=e;return l`
    <div class="card">
  ...
  `}function Nh(e){const{props:t,slack:n,accountCountLabel:s}=e;return l`
    <div class="card">
...
${e.snapshot?JSON.stringify(e.snapshot,null,2):"No snapshot yet."}
  ...
  `}const kn=(e,t)=>{const n=e._$AN;if(n===void 0)return!1;for(const s of n)s._$AO?.(t,!1),kn(s,t);return!0},vs=e=>{let t,n;do{if((t=e._$AM)===void 0)break;n=t._$AN,n.delete(e),e=t}while(n?.size===0)},Jl=e=>{for(let t;t=e._$AM;e=t){let n=t._$AN;if(n===void 0)t._$AN=n=new Set;else if(n.has(e))break;n.add(e),Zh(t)}};function Qh(e){this._$AN!==void 0?(vs(this),this._$AM=e,Jl(this)):this._$AM=e}function Yh(e,t=!1,n=0){const s=this._$AH,i=this._$AN;if(i!==void 0&&i.size!==0)if(t)if(Array.isArray(s))for(let a=n;a<s.length;a++)kn(s[a],!1),vs(s[a]);else s!=null&&(kn(s,!1),vs(s));else kn(this,e)}const Zh=e=>{e.type==ba.CHILD&&(e._$AP??=Yh,e._$AQ??=Qh)};class Xh extends xa{constructor(){super(...arguments),this._$AN=void 0}_$AT(t,n,s){super._$AT(t,n,s),Jl(this),this.isConnected=t._$AU}_$AO(t,n=!0){t!==this.isConnected&&(this.isConnected=t,t?this.reconnected?.():this.disconnected?.()),n&&(kn(this,t),vs(this))}setValue(t){if(of(this._$Ct))this._$Ct._$AI(t,this);else{const n=[...this._$Ct._$AH];n[this._$Ci]=t,this._$Ct._$AI(n,this,0)}}disconnected(){}reconnected(){}}const ri=new WeakMap,Jh=ya(class extends Xh{render(e){return v}update(e,[t]){const n=t!==this.G;return n&&this.G!==void 0&&this.rt(void 0),(n||this.lt!==this.ct)&&(this.G=t,this.ht=e.options?.host,this.rt(this.ct=e.element)),v}rt(e){if(this.isConnected||(e=void 0),typeof this.G=="function"){const t=this.ht??globalThis;let n=ri.get(t);n===void 0&&(n=new WeakMap,ri.set(t,n)),n.get(this.G)!==void 0&&this.G.call(this.ht,void 0),n.set(this.G,e),e!==void 0&&this.G.call(this.ht,e)}else this.G.value=e}get lt(){return typeof this.G=="function"?ri.get(this.ht??globalThis)?.get(this.G):this.G?.value}disconnected(){this.lt===this.ct&&this.rt(void 0)}reconnected(){this.rt(this.ct)}});class Li extends xa{constructor(t){if(super(t),this.it=v,t.type!==ba.CHILD)throw Error(this.constructor.directiveName+"() can only be used in child bindings")}render(t){if(t===v||t==null)return this._t=void 0,this.it=t;if(t===ht)return t;if(typeof t!="string")throw Error(this.constructor.directiveName+"() called with a non-string value");if(t===this.it)return this._t;this.it=t;const n=[t];return n.raw=n,this._t={_$litType$:this.constructor.resultType,strings:n,values:[]}}}Li.directiveName="unsafeHTML",Li.resultType=1;const Mi=ya(Li);const{entries:ec,setPrototypeOf:Zo,isFrozen:ev,getPrototypeOf:tv,getOwnPropertyDescriptor:nv}=Object;let{freeze:we,seal:Re,create:Ii}=Object,{apply:Ri,construct:Pi}=typeof Reflect<"u"&&Reflect;we||(we=function(t){return t});Re||(Re=function(t){return t});Ri||(Ri=function(t,n){for(var s=arguments.length,i=new Array(s>2?s-2:0),a=2;a<s;a++)i[a-2]=arguments[a];return t.apply(n,i)});Pi||(Pi=function(t){for(var n=arguments.length,s=new Array(n>1?n-1:0),i=1;i<n;i++)s[i-1]=arguments[i];return new t(...s)});const Gn=ke(Array.prototype.forEach),sv=ke(Array.prototype.lastIndexOf),Xo=ke(Array.prototype.pop),cn=ke(Array.prototype.push),iv=ke(Array.prototype.splice),os=ke(String.prototype.toLowerCase),li=ke(String.prototype.toString),ci=ke(String.prototype.match),dn=ke(String.prototype.replace),av=ke(String.prototype.indexOf),ov=ke(String.prototype.trim),Pe=ke(Object.prototype.hasOwnProperty),xe=ke(RegExp.prototype.test),un=rv(TypeError);function ke(e){return function(t){t instanceof RegExp&&(t.lastIndex=0);for(var n=arguments.length,s=new Array(n>1?n-1:0),i=1;i<n;i++)s[i-1]=arguments[i];return Ri(e,t,s)}}function rv(e){return function(){for(var t=arguments.length,n=new Array(t),s=0;s<t;s++)n[s]=arguments[s];return Pi(e,n)}}function W(e,t){let n=arguments.length>2&&arguments[2]!==void 0?arguments[2]:os;Zo&&Zo(e,null);let s=t.length;for(;s--;){let i=t[s];if(typeof i=="string"){const a=n(i);a!==i&&(ev(t)||(t[s]=a),i=a)}e[i]=!0}return e}function lv(e){for(let t=0;t<e.length;t++)Pe(e,t)||(e[t]=null);return e}function He(e){const t=Ii(null);for(const[n,s]of ec(e))Pe(e,n)&&(Array.isArray(s)?t[n]=lv(s):s&&typeof s=="object"&&s.constructor===Object?t[n]=He(s):t[n]=s);return t}function gn(e,t){for(;e!==null;){const s=nv(e,t);if(s){if(s.get)return ke(s.get);if(typeof s.value=="function")return ke(s.value)}e=tv(e)}function n(){return null}return n}const Jo=we(["a","abbr","acronym","address","area","article","aside","audio","b","bdi","bdo","big","blink","blockquote","body","br","button","canvas","caption","center","cite","code","col","colgroup","content","data","datalist","dd","decorator","del","details","dfn","dialog","dir","div","dl","dt","element","em","fieldset","figcaption","figure","font","footer","form","h1","h2","h3","h4","h5","h6","head","header","hgroup","hr","html","i","img","input","ins","kbd","label","legend","li","main","map","mark","marquee","menu","menuitem","meter","nav","nobr","ol","optgroup","option","output","p","picture","pre","progress","q","rp","rt","ruby","s","samp","search","section","select","shadow","slot","small","source","spacer","span","strike","strong","style","sub","summary","sup","table","tbody","td","template","textarea","tfoot","th","thead","time","tr","track","tt","u","ul","var","video","wbr"]),di=we(["svg","a","altglyph","altglyphdef","altglyphitem","animatecolor","animatemotion","animatetransform","circle","clippath","defs","desc","ellipse","enterkeyhint","exportparts","filter","font","g","glyph","glyphref","hkern","image","inputmode","line","lineargradient","marker","mask","metadata","mpath","part","path","pattern","polygon","polyline","radialgradient","rect","stop","style","switch","symbol","text","textpath","title","tref","tspan","view","vkern"]),ui=we(["feBlend","feColorMatrix","feComponentTransfer","feComposite","feConvolveMatrix","feDiffuseLighting","feDisplacementMap","feDistantLight","feDropShadow","feFlood","feFuncA","feFuncB","feFuncG","feFuncR","feGaussianBlur","feImage","feMerge","feMergeNode","feMorphology","feOffset","fePointLight","feSpecularLighting","feSpotLight","feTile","feTurbulence"]),cv=we(["animate","color-profile","cursor","discard","font-face","font-face-format","font-face-name","font-face-src","font-face-uri","foreignobject","hatch","hatchpath","mesh","meshgradient","meshpatch","meshrow","missing-glyph","script","set","solidcolor","unknown","use"]),gi=we(["math","menclose","merror","mfenced","mfrac","mglyph","mi","mlabeledtr","mmultiscripts","mn","mo","mover","mpadded","mphantom","mroot","mrow","ms","mspace","msqrt","mstyle","msub","msup","msubsup","mtable","mtd","mtext","mtr","munder","munderover","mprescripts"]),dv=we(["maction","maligngroup","malignmark","mlongdiv","mscarries","mscarry","msgroup","mstack","msline","msrow","semantics","annotation","annotation-xml","mprescripts","none"]),er=we(["#text"]),tr=we(["accept","action","align","alt","autocapitalize","autocomplete","autopictureinpicture","autoplay","background","bgcolor","border","capture","cellpadding","cellspacing","checked","cite","class","clear","color","cols","colspan","controls","controlslist","coords","crossorigin","datetime","decoding","default","dir","disabled","disablepictureinpicture","disableremoteplayback","download","draggable","enctype","enterkeyhint","exportparts","face","for","headers","height","hidden","high","href","hreflang","id","inert","inputmode","integrity","ismap","kind","label","lang","list","loading","loop","low","max","maxlength","media","method","min","minlength","multiple","muted","name","nonce","noshade","novalidate","nowrap","open","optimum","part","pattern","placeholder","playsinline","popover","popovertarget","popovertargetaction","poster","preload","pubdate","radiogroup","readonly","rel","required","rev","reversed","role","rows","rowspan","spellcheck","scope","selected","shape","size","sizes","slot","span","srclang","start","src","srcset","step","style","summary","tabindex","title","translate","type","usemap","valign","value","width","wrap","xmlns","slot"]),pi=we(["accent-height","accumulate","additive","alignment-baseline","amplitude","ascent","attributename","attributetype","azimuth","basefrequency","baseline-shift","begin","bias","by","class","clip","clippathunits","clip-path","clip-rule","color","color-interpolation","color-interpolation-filters","color-profile","color-rendering","cx","cy","d","dx","dy","diffuseconstant","direction","display","divisor","dur","edgemode","elevation","end","exponent","fill","fill-opacity","fill-rule","filter","filterunits","flood-color","flood-opacity","font-family","font-size","font-size-adjust","font-stretch","font-style","font-variant","font-weight","fx","fy","g1","g2","glyph-name","glyphref","gradientunits","gradienttransform","height","href","id","image-rendering","in","in2","intercept","k","k1","k2","k3","k4","kerning","keypoints","keysplines","keytimes","lang","lengthadjust","letter-spacing","kernelmatrix","kernelunitlength","lighting-color","local","marker-end","marker-mid","marker-start","markerheight","markerunits","markerwidth","maskcontentunits","maskunits","max","mask","mask-type","media","method","mode","min","name","numoctaves","offset","operator","opacity","order","orient","orientation","origin","overflow","paint-order","path","pathlength","patterncontentunits","patterntransform","patternunits","points","preservealpha","preserveaspectratio","primitiveunits","r","rx","ry","radius","refx","refy","repeatcount","repeatdur","restart","result","rotate","scale","seed","shape-rendering","slope","specularconstant","specularexponent","spreadmethod","startoffset","stddeviation","stitchtiles","stop-color","stop-opacity","stroke-dasharray","stroke-dashoffset","stroke-linecap","stroke-linejoin","stroke-miterlimit","stroke-opacity","stroke","stroke-width","style","surfacescale","systemlanguage","tabindex","tablevalues","targetx","targety","transform","transform-origin","text-anchor","text-decoration","text-rendering","textlength","type","u1","u2","unicode","values","viewbox","visibility","version","vert-adv-y","vert-origin-x","vert-origin-y","width","word-spacing","wrap","writing-mode","xchannelselector","ychannelselector","x","x1","x2","xmlns","y","y1","y2","z","zoomandpan"]),nr=we(["accent","accentunder","align","bevelled","close","columnsalign","columnlines","columnspan","denomalign","depth","dir","display","displaystyle","encoding","fence","frame","height","href","id","largeop","length","linethickness","lspace","lquote","mathbackground","mathcolor","mathsize","mathvariant","maxsize","minsize","movablelimits","notation","numalign","open","rowalign","rowlines","rowspacing","rowspan","rspace","rquote","scriptlevel","scriptminsize","scriptsizemultiplier","selection","separator","separators","stretchy","subscriptshift","supscriptshift","symmetric","voffset","width","xmlns"]),Qn=we(["xlink:href","xml:id","xlink:title","xml:space","xmlns:xlink"]),uv=Re(/\{\{[\w\W]*|[\w\W]*\}\}/gm),gv=Re(/<%[\w\W]*|[\w\W]*%>/gm),pv=Re(/\$\{[\w\W]*/gm),fv=Re(/^data-[\-\w.\u00B7-\uFFFF]+$/),hv=Re(/^aria-[\-\w]+$/),tc=Re(/^(?:(?:(?:f|ht)tps?|mailto|tel|callto|sms|cid|xmpp|matrix):|[^a-z]|[a-z+.\-]+(?:[^a-z+.\-:]|$))/i),vv=Re(/^(?:\w+script|data):/i),mv=Re(/[\u0000-\u0020\u00A0\u1680\u180E\u2000-\u2029\u205F\u3000]/g),nc=Re(/^html$/i),bv=Re(/^[a-z][.\w]*(-[.\w]+)+$/i);var sr=Object.freeze({__proto__:null,ARIA_ATTR:hv,ATTR_WHITESPACE:mv,CUSTOM_ELEMENT:bv,DATA_ATTR:fv,DOCTYPE_NAME:nc,ERB_EXPR:gv,IS_ALLOWED_URI:tc,IS_SCRIPT_OR_DATA:vv,MUSTACHE_EXPR:uv,TMPLIT_EXPR:pv});const pn={element:1,text:3,progressingInstruction:7,comment:8,document:9},yv=function(){return typeof window>"u"?null:window},xv=function(t,n){if(typeof t!="object"||typeof t.createPolicy!="function")return null;let s=null;const i="data-tt-policy-suffix";n&&n.hasAttribute(i)&&(s=n.getAttribute(i));const a="dompurify"+(s?"#"+s:"");try{return t.createPolicy(a,{createHTML(o){return o},createScriptURL(o){return o}})}catch{return console.warn("TrustedTypes policy "+a+" could not be created."),null}},ir=function(){return{afterSanitizeAttributes:[],afterSanitizeElements:[],afterSanitizeShadowDOM:[],beforeSanitizeAttributes:[],beforeSanitizeElements:[],beforeSanitizeShadowDOM:[],uponSanitizeAttribute:[],uponSanitizeElement:[],uponSanitizeShadowNode:[]}};function sc(){let e=arguments.length>0&&arguments[0]!==void 0?arguments[0]:yv();const t=z=>sc(z);if(t.version="3.3.1",t.removed=[],!e||!e.document||e.document.nodeType!==pn.document||!e.Element)return t.isSupported=!1,t;let{document:n}=e;const s=n,i=s.currentScript,{DocumentFragment:a,HTMLTemplateElement:o,Node:r,Element:c,NodeFilter:d,NamedNodeMap:g=e.NamedNodeMap||e.MozNamedAttrMap,HTMLFormElement:p,DOMParser:h,trustedTypes:u}=e,f=c.prototype,m=gn(f,"cloneNode"),w=gn(f,"remove"),S=gn(f,"nextSibling"),C=gn(f,"childNodes"),k=gn(f,"parentNode");if(typeof o=="function"){const z=n.createElement("template");z.content&&z.content.ownerDocument&&(n=z.content.ownerDocument)}let A,E="";const{implementation:T,createNodeIterator:I,createDocumentFragment:H,getElementsByTagName:Z}=n,{importNode:te}=s;let R=ir();t.isSupported=typeof ec=="function"&&typeof k=="function"&&T&&T.createHTMLDocument!==void 0;const{MUSTACHE_EXPR:q,ERB_EXPR:ne,TMPLIT_EXPR:oe,DATA_ATTR:_,ARIA_ATTR:j,IS_SCRIPT_OR_DATA:Q,ATTR_WHITESPACE:re,CUSTOM_ELEMENT:pe}=sr;let{IS_ALLOWED_URI:M}=sr,D=null;const N=W({},[...Jo,...di,...ui,...gi,...er]);let K=null;const ce=W({},[...tr,...pi,...nr,...Qn]);let X=Object.seal(Ii(null,{tagNameCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},attributeNameCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},allowCustomizedBuiltInElements:{writable:!0,configurable:!1,enumerable:!0,value:!1}})),se=null,V=null;const U=Object.seal(Ii(null,{tagCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},attributeCheck:{writable:!0,configurable:!1,enumerable:!0,value:null}}));let ie=!0,le=!0,fe=!1,Te=!0,Ke=!1,it=!0,he=!1,Oe=!1,We=!1,qe=!1,Ve=!1,at=!1,ot=!0,xt=!1;const Fs="user-content-";let Kt=!0,rt=!1,Be={},Se=null;const an=W({},["annotation-xml","audio","colgroup","desc","foreignobject","head","iframe","math","mi","mn","mo","ms","mtext","noembed","noframes","noscript","plaintext","script","style","svg","template","thead","title","video","xmp"]);let Wt=null;const lt=W({},["audio","video","img","source","image","track"]);let Os=null;const Oa=W({},["alt","class","for","id","label","name","pattern","placeholder","role","summary","title","value","style","xmlns"]),Nn="http://www.w3.org/1998/Math/MathML",Fn="http://www.w3.org/2000/svg",Ge="http://www.w3.org/1999/xhtml";let qt=Ge,Bs=!1,zs=null;const Tc=W({},[Nn,Fn,Ge],li);let On=W({},["mi","mo","mn","ms","mtext"]),Bn=W({},["annotation-xml"]);const _c=W({},["title","style","font","a","script"]);let on=null;const Ec=["application/xhtml+xml","text/html"],Lc="text/html";let ue=null,Vt=null;const Mc=n.createElement("form"),Ba=function(b){return b instanceof RegExp||b instanceof Function},Us=function(){let b=arguments.length>0&&arguments[0]!==void 0?arguments[0]:{};if(!(Vt&&Vt===b)){if((!b||typeof b!="object")&&(b={}),b=He(b),on=Ec.indexOf(b.PARSER_MEDIA_TYPE)===-1?Lc:b.PARSER_MEDIA_TYPE,ue=on==="application/xhtml+xml"?li:os,D=Pe(b,"ALLOWED_TAGS")?W({},b.ALLOWED_TAGS,ue):N,K=Pe(b,"ALLOWED_ATTR")?W({},b.ALLOWED_ATTR,ue):ce,zs=Pe(b,"ALLOWED_NAMESPACES")?W({},b.ALLOWED_NAMESPACES,li):Tc,Os=Pe(b,"ADD_URI_SAFE_ATTR")?W(He(Oa),b.ADD_URI_SAFE_ATTR,ue):Oa,Wt=Pe(b,"ADD_DATA_URI_TAGS")?W(He(lt),b.ADD_DATA_URI_TAGS,ue):lt,Se=Pe(b,"FORBID_CONTENTS")?W({},b.FORBID_CONTENTS,ue):an,se=Pe(b,"FORBID_TAGS")?W({},b.FORBID_TAGS,ue):He({}),V=Pe(b,"FORBID_ATTR")?W({},b.FORBID_ATTR,ue):He({}),Be=Pe(b,"USE_PROFILES")?b.USE_PROFILES:!1,ie=b.ALLOW_ARIA_ATTR!==!1,le=b.ALLOW_DATA_ATTR!==!1,fe=b.ALLOW_UNKNOWN_PROTOCOLS||!1,Te=b.ALLOW_SELF_CLOSE_IN_ATTR!==!1,Ke=b.SAFE_FOR_TEMPLATES||!1,it=b.SAFE_FOR_XML!==!1,he=b.WHOLE_DOCUMENT||!1,qe=b.RETURN_DOM||!1,Ve=b.RETURN_DOM_FRAGMENT||!1,at=b.RETURN_TRUSTED_TYPE||!1,We=b.FORCE_BODY||!1,ot=b.SANITIZE_DOM!==!1,xt=b.SANITIZE_NAMED_PROPS||!1,Kt=b.KEEP_CONTENT!==!1,rt=b.IN_PLACE||!1,M=b.ALLOWED_URI_REGEXP||tc,qt=b.NAMESPACE||Ge,On=b.MATHML_TEXT_INTEGRATION_POINTS||On,Bn=b.HTML_INTEGRATION_POINTS||Bn,X=b.CUSTOM_ELEMENT_HANDLING||{},b.CUSTOM_ELEMENT_HANDLING&&Ba(b.CUSTOM_ELEMENT_HANDLING.tagNameCheck)&&(X.tagNameCheck=b.CUSTOM_ELEMENT_HANDLING.tagNameCheck),b.CUSTOM_ELEMENT_HANDLING&&Ba(b.CUSTOM_ELEMENT_HANDLING.attributeNameCheck)&&(X.attributeNameCheck=b.CUSTOM_ELEMENT_HANDLING.attributeNameCheck),b.CUSTOM_ELEMENT_HANDLING&&typeof b.CUSTOM_ELEMENT_HANDLING.allowCustomizedBuiltInElements=="boolean"&&(X.allowCustomizedBuiltInElements=b.CUSTOM_ELEMENT_HANDLING.allowCustomizedBuiltInElements),Ke&&(le=!1),Ve&&(qe=!0),Be&&(D=W({},er),K=[],Be.html===!0&&(W(D,Jo),W(K,tr)),Be.svg===!0&&(W(D,di),W(K,pi),W(K,Qn)),Be.svgFilters===!0&&(W(D,ui),W(K,pi),W(K,Qn)),Be.mathMl===!0&&(W(D,gi),W(K,nr),W(K,Qn))),b.ADD_TAGS&&(typeof b.ADD_TAGS=="function"?U.tagCheck=b.ADD_TAGS:(D===N&&(D=He(D)),W(D,b.ADD_TAGS,ue))),b.ADD_ATTR&&(typeof b.ADD_ATTR=="function"?U.attributeCheck=b.ADD_ATTR:(K===ce&&(K=He(K)),W(K,b.ADD_ATTR,ue))),b.ADD_URI_SAFE_ATTR&&W(Os,b.ADD_URI_SAFE_ATTR,ue),b.FORBID_CONTENTS&&(Se===an&&(Se=He(Se)),W(Se,b.FORBID_CONTENTS,ue)),b.ADD_FORBID_CONTENTS&&(Se===an&&(Se=He(Se)),W(Se,b.ADD_FORBID_CONTENTS,ue)),Kt&&(D["#text"]=!0),he&&W(D,["html","head","body"]),D.table&&(W(D,["tbody"]),delete se.tbody),b.TRUSTED_TYPES_POLICY){if(typeof b.TRUSTED_TYPES_POLICY.createHTML!="function")throw un('TRUSTED_TYPES_POLICY configuration option must provide a "createHTML" hook.');if(typeof b.TRUSTED_TYPES_POLICY.createScriptURL!="function")throw un('TRUSTED_TYPES_POLICY configuration option must provide a "createScriptURL" hook.');A=b.TRUSTED_TYPES_POLICY,E=A.createHTML("")}else A===void 0&&(A=xv(u,i)),A!==null&&typeof E=="string"&&(E=A.createHTML(""));we&&we(b),Vt=b}},za=W({},[...di,...ui,...cv]),Ua=W({},[...gi,...dv]),Ic=function(b){let L=k(b);(!L||!L.tagName)&&(L={namespaceURI:qt,tagName:"template"});const O=os(b.tagName),ae=os(L.tagName);return zs[b.namespaceURI]?b.namespaceURI===Fn?L.namespaceURI===Ge?O==="svg":L.namespaceURI===Nn?O==="svg"&&(ae==="annotation-xml"||On[ae]):!!za[O]:b.namespaceURI===Nn?L.namespaceURI===Ge?O==="math":L.namespaceURI===Fn?O==="math"&&Bn[ae]:!!Ua[O]:b.namespaceURI===Ge?L.namespaceURI===Fn&&!Bn[ae]||L.namespaceURI===Nn&&!On[ae]?!1:!Ua[O]&&(_c[O]||!za[O]):!!(on==="application/xhtml+xml"&&zs[b.namespaceURI]):!1},ze=function(b){cn(t.removed,{element:b});try{k(b).removeChild(b)}catch{w(b)}},$t=function(b,L){try{cn(t.removed,{attribute:L.getAttributeNode(b),from:L})}catch{cn(t.removed,{attribute:null,from:L})}if(L.removeAttribute(b),b==="is")if(qe||Ve)try{ze(L)}catch{}else try{L.setAttribute(b,"")}catch{}},Ha=function(b){let L=null,O=null;if(We)b="<remove></remove>"+b;else{const de=ci(b,/^[\r\n\t ]+/);O=de&&de[0]}on==="application/xhtml+xml"&&qt===Ge&&(b='<html xmlns="http://www.w3.org/1999/xhtml"><head></head><body>'+b+"</body></html>");const ae=A?A.createHTML(b):b;if(qt===Ge)try{L=new h().parseFromString(ae,on)}catch{}if(!L||!L.documentElement){L=T.createDocument(qt,"template",null);try{L.documentElement.innerHTML=Bs?E:ae}catch{}}const be=L.body||L.documentElement;return b&&O&&be.insertBefore(n.createTextNode(O),be.childNodes[0]||null),qt===Ge?Z.call(L,he?"html":"body")[0]:he?L.documentElement:be},ja=function(b){return I.call(b.ownerDocument||b,b,d.SHOW_ELEMENT|d.SHOW_COMMENT|d.SHOW_TEXT|d.SHOW_PROCESSING_INSTRUCTION|d.SHOW_CDATA_SECTION,null)},Hs=function(b){return b instanceof p&&(typeof b.nodeName!="string"||typeof b.textContent!="string"||typeof b.removeChild!="function"||!(b.attributes instanceof g)||typeof b.removeAttribute!="function"||typeof b.setAttribute!="function"||typeof b.namespaceURI!="string"||typeof b.insertBefore!="function"||typeof b.hasChildNodes!="function")},Ka=function(b){return typeof r=="function"&&b instanceof r};function Qe(z,b,L){Gn(z,O=>{O.call(t,b,L,Vt)})}const Wa=function(b){let L=null;if(Qe(R.beforeSanitizeElements,b,null),Hs(b))return ze(b),!0;const O=ue(b.nodeName);if(Qe(R.uponSanitizeElement,b,{tagName:O,allowedTags:D}),it&&b.hasChildNodes()&&!Ka(b.firstElementChild)&&xe(/<[/\w!]/g,b.innerHTML)&&xe(/<[/\w!]/g,b.textContent)||b.nodeType===pn.progressingInstruction||it&&b.nodeType===pn.comment&&xe(/<[/\w]/g,b.data))return ze(b),!0;if(!(U.tagCheck instanceof Function&&U.tagCheck(O))&&(!D[O]||se[O])){if(!se[O]&&Va(O)&&(X.tagNameCheck instanceof RegExp&&xe(X.tagNameCheck,O)||X.tagNameCheck instanceof Function&&X.tagNameCheck(O)))return!1;if(Kt&&!Se[O]){const ae=k(b)||b.parentNode,be=C(b)||b.childNodes;if(be&&ae){const de=be.length;for(let Ae=de-1;Ae>=0;--Ae){const Ye=m(be[Ae],!0);Ye.__removalCount=(b.__removalCount||0)+1,ae.insertBefore(Ye,S(b))}}}return ze(b),!0}return b instanceof c&&!Ic(b)||(O==="noscript"||O==="noembed"||O==="noframes")&&xe(/<\/no(script|embed|frames)/i,b.innerHTML)?(ze(b),!0):(Ke&&b.nodeType===pn.text&&(L=b.textContent,Gn([q,ne,oe],ae=>{L=dn(L,ae," ")}),b.textContent!==L&&(cn(t.removed,{element:b.cloneNode()}),b.textContent=L)),Qe(R.afterSanitizeElements,b,null),!1)},qa=function(b,L,O){if(ot&&(L==="id"||L==="name")&&(O in n||O in Mc))return!1;if(!(le&&!V[L]&&xe(_,L))){if(!(ie&&xe(j,L))){if(!(U.attributeCheck instanceof Function&&U.attributeCheck(L,b))){if(!K[L]||V[L]){if(!(Va(b)&&(X.tagNameCheck instanceof RegExp&&xe(X.tagNameCheck,b)||X.tagNameCheck instanceof Function&&X.tagNameCheck(b))&&(X.attributeNameCheck instanceof RegExp&&xe(X.attributeNameCheck,L)||X.attributeNameCheck instanceof Function&&X.attributeNameCheck(L,b))||L==="is"&&X.allowCustomizedBuiltInElements&&(X.tagNameCheck instanceof RegExp&&xe(X.tagNameCheck,O)||X.tagNameCheck instanceof Function&&X.tagNameCheck(O))))return!1}else if(!Os[L]){if(!xe(M,dn(O,re,""))){if(!((L==="src"||L==="xlink:href"||L==="href")&&b!=="script"&&av(O,"data:")===0&&Wt[b])){if(!(fe&&!xe(Q,dn(O,re,"")))){if(O)return!1}}}}}}}return!0},Va=function(b){return b!=="annotation-xml"&&ci(b,pe)},Ga=function(b){Qe(R.beforeSanitizeAttributes,b,null);const{attributes:L}=b;if(!L||Hs(b))return;const O={attrName:"",attrValue:"",keepAttr:!0,allowedAttributes:K,forceKeepAttr:void 0};let ae=L.length;for(;ae--;){const be=L[ae],{name:de,namespaceURI:Ae,value:Ye}=be,Gt=ue(de),js=Ye;let ve=de==="value"?js:ov(js);if(O.attrName=Gt,O.attrValue=ve,O.keepAttr=!0,O.forceKeepAttr=void 0,Qe(R.uponSanitizeAttribute,b,O),ve=O.attrValue,xt&&(Gt==="id"||Gt==="name")&&($t(de,b),ve=Fs+ve),it&&xe(/((--!?|])>)|<\/(style|title|textarea)/i,ve)){$t(de,b);continue}if(Gt==="attributename"&&ci(ve,"href")){$t(de,b);continue}if(O.forceKeepAttr)continue;if(!O.keepAttr){$t(de,b);continue}if(!Te&&xe(/\/>/i,ve)){$t(de,b);continue}Ke&&Gn([q,ne,oe],Ya=>{ve=dn(ve,Ya," ")});const Qa=ue(b.nodeName);if(!qa(Qa,Gt,ve)){$t(de,b);continue}if(A&&typeof u=="object"&&typeof u.getAttributeType=="function"&&!Ae)switch(u.getAttributeType(Qa,Gt)){case"TrustedHTML":{ve=A.createHTML(ve);break}case"TrustedScriptURL":{ve=A.createScriptURL(ve);break}}if(ve!==js)try{Ae?b.setAttributeNS(Ae,de,ve):b.setAttribute(de,ve),Hs(b)?ze(b):Xo(t.removed)}catch{$t(de,b)}}Qe(R.afterSanitizeAttributes,b,null)},Rc=function z(b){let L=null;const O=ja(b);for(Qe(R.beforeSanitizeShadowDOM,b,null);L=O.nextNode();)Qe(R.uponSanitizeShadowNode,L,null),Wa(L),Ga(L),L.content instanceof a&&z(L.content);Qe(R.afterSanitizeShadowDOM,b,null)};return t.sanitize=function(z){let b=arguments.length>1&&arguments[1]!==void 0?arguments[1]:{},L=null,O=null,ae=null,be=null;if(Bs=!z,Bs&&(z="<!-->"),typeof z!="string"&&!Ka(z))if(typeof z.toString=="function"){if(z=z.toString(),typeof z!="string")throw un("dirty is not a string, aborting")}else throw un("toString is not a function");if(!t.isSupported)return z;if(Oe||Us(b),t.removed=[],typeof z=="string"&&(rt=!1),rt){if(z.nodeName){const Ye=ue(z.nodeName);if(!D[Ye]||se[Ye])throw un("root node is forbidden and cannot be sanitized in-place")}}else if(z instanceof r)L=Ha("<!---->"),O=L.ownerDocument.importNode(z,!0),O.nodeType===pn.element&&O.nodeName==="BODY"||O.nodeName==="HTML"?L=O:L.appendChild(O);else{if(!qe&&!Ke&&!he&&z.indexOf("<")===-1)return A&&at?A.createHTML(z):z;if(L=Ha(z),!L)return qe?null:at?E:""}L&&We&&ze(L.firstChild);const de=ja(rt?z:L);for(;ae=de.nextNode();)Wa(ae),Ga(ae),ae.content instanceof a&&Rc(ae.content);if(rt)return z;if(qe){if(Ve)for(be=H.call(L.ownerDocument);L.firstChild;)be.appendChild(L.firstChild);else be=L;return(K.shadowroot||K.shadowrootmode)&&(be=te.call(s,be,!0)),be}let Ae=he?L.outerHTML:L.innerHTML;return he&&D["!doctype"]&&L.ownerDocument&&L.ownerDocument.doctype&&L.ownerDocument.doctype.name&&xe(nc,L.ownerDocument.doctype.name)&&(Ae="<!DOCTYPE "+L.ownerDocument.doctype.name+`>
`+Ae),Ke&&Gn([q,ne,oe],Ye=>{Ae=dn(Ae,Ye," ")}),A&&at?A.createHTML(Ae):Ae},t.setConfig=function(){let z=arguments.length>0&&arguments[0]!==void 0?arguments[0]:{};Us(z),Oe=!0},t.clearConfig=function(){Vt=null,Oe=!1},t.isValidAttribute=function(z,b,L){Vt||Us({});const O=ue(z),ae=ue(b);return qa(O,ae,L)},t.addHook=function(z,b){typeof b=="function"&&cn(R[z],b)},t.removeHook=function(z,b){if(b!==void 0){const L=sv(R[z],b);return L===-1?void 0:iv(R[z],L,1)[0]}return Xo(R[z])},t.removeHooks=function(z){R[z]=[]},t.removeAllHooks=function(){R=ir()},t}var Di=sc();function ka(){return{async:!1,breaks:!1,extensions:null,gfm:!0,hooks:null,pedantic:!1,renderer:null,silent:!1,tokenizer:null,walkTokens:null}}var jt=ka();function ic(e){jt=e}var It={exec:()=>null};function G(e,t=""){let n=typeof e=="string"?e:e.source,s={replace:(i,a)=>{let o=typeof a=="string"?a:a.source;return o=o.replace($e.caret,"$1"),n=n.replace(i,o),s},getRegex:()=>new RegExp(n,t)};return s}var $v=(()=>{try{return!!new RegExp("(?<=1)(?<!1)")}catch{return!1}})(),$e={codeRemoveIndent:/^(?: {1,4}| {0,3}\t)/gm,outputLinkReplace:/\\([\[\]])/g,indentCodeCompensation:/^(\s+)(?:```)/,beginningSpace:/^\s+/,endingHash:/#$/,startingSpaceChar:/^ /,endingSpaceChar:/ $/,nonSpaceChar:/[^ ]/,newLineCharGlobal:/\n/g,tabCharGlobal:/\t/g,multipleSpaceGlobal:/\s+/g,blankLine:/^[ \t]*$/,doubleBlankLine:/\n[ \t]*\n[ \t]*$/,blockquoteStart:/^ {0,3}>/,blockquoteSetextReplace:/\n {0,3}((?:=+|-+) *)(?=\n|$)/g,blockquoteSetextReplace2:/^ {0,3}>[ \t]?/gm,listReplaceNesting:/^ {1,4}(?=( {4})*[^ ])/g,listIsTask:/^\[[ xX]\] +\S/,listReplaceTask:/^\[[ xX]\] +/,listTaskCheckbox:/\[[ xX]\]/,anyLine:/\n.*\n/,hrefBrackets:/^<(.*)>$/,tableDelimiter:/[:|]/,tableAlignChars:/^\||\| *$/g,tableRowBlankLine:/\n[ \t]*$/,tableAlignRight:/^ *-+: *$/,tableAlignCenter:/^ *:-+: *$/,tableAlignLeft:/^ *:-+ *$/,startATag:/^<a /i,endATag:/^<\/a>/i,startPreScriptTag:/^<(pre|code|kbd|script)(\s|>)/i,endPreScriptTag:/^<\/(pre|code|kbd|script)(\s|>)/i,startAngleBracket:/^</,endAngleBracket:/>$/,pedanticHrefTitle:/^([^'"]*[^\s])\s+(['"])(.*)\2/,unicodeAlphaNumeric:/[\p{L}\p{N}]/u,escapeTest:/[&<>"']/,escapeReplace:/[&<>"']/g,escapeTestNoEncode:/[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/,escapeReplaceNoEncode:/[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/g,unescapeTest:/&(#(?:\d+)|(?:#x[0-9A-Fa-f]+)|(?:\w+));?/ig,caret:/(^|[^\[])\^/g,percentDecode:/%25/g,findPipe:/\|/g,splitPipe:/ \|/,slashPipe:/\\\|/g,carriageReturn:/\r\n|\r/g,spaceLine:/^ +$/gm,notSpaceStart:/^\S*/,endingNewline:/\n$/,listItemRegex:e=>new RegExp(`^( {0,3}${e})((?:[	 ][^\\n]*)?(?:\\n|$))`),nextBulletRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`),hrRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`),fencesBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}(?:\`\`\`|~~~)`),headingBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}#`),htmlBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}<(?:[a-z].*>|!--)`,"i"),blockquoteBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}>`)},wv=/^(?:[ \t]*(?:\n|$))+/,kv=/^((?: {4}| {0,3}\t)[^\n]+(?:\n(?:[ \t]*(?:\n|$))*)?)+/,Sv=/^ {0,3}(`{3,}(?=[^`\n]*(?:\n|$))|~{3,})([^\n]*)(?:\n|$)(?:|([\s\S]*?)(?:\n|$))(?: {0,3}\1[~`]* *(?=\n|$)|$)/,Dn=/^ {0,3}((?:-[\t ]*){3,}|(?:_[ \t]*){3,}|(?:\*[ \t]*){3,})(?:\n+|$)/,Av=/^ {0,3}(#{1,6})(?=\s|$)(.*)(?:\n+|$)/,Sa=/ {0,3}(?:[*+-]|\d{1,9}[.)])/,ac=/^(?!bull |blockCode|fences|blockquote|heading|html|table)((?:.|\n(?!\s*?\n|bull |blockCode|fences|blockquote|heading|html|table))+?)\n {0,3}(=+|-+) *(?:\n+|$)/,oc=G(ac).replace(/bull/g,Sa).replace(/blockCode/g,/(?: {4}| {0,3}\t)/).replace(/fences/g,/ {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g,/ {0,3}>/).replace(/heading/g,/ {0,3}#{1,6}/).replace(/html/g,/ {0,3}<[^\n>]+>\n/).replace(/\|table/g,"").getRegex(),Cv=G(ac).replace(/bull/g,Sa).replace(/blockCode/g,/(?: {4}| {0,3}\t)/).replace(/fences/g,/ {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g,/ {0,3}>/).replace(/heading/g,/ {0,3}#{1,6}/).replace(/html/g,/ {0,3}<[^\n>]+>\n/).replace(/table/g,/ {0,3}\|?(?:[:\- ]*\|)+[\:\- ]*\n/).getRegex(),Aa=/^([^\n]+(?:\n(?!hr|heading|lheading|blockquote|fences|list|html|table| +\n)[^\n]+)*)/,Tv=/^[^\n]+/,Ca=/(?!\s*\])(?:\\[\s\S]|[^\[\]\\])+/,_v=G(/^ {0,3}\[(label)\]: *(?:\n[ \t]*)?([^<\s][^\s]*|<.*?>)(?:(?: +(?:\n[ \t]*)?| *\n[ \t]*)(title))? *(?:\n+|$)/).replace("label",Ca).replace("title",/(?:"(?:\\"?|[^"\\])*"|'[^'\n]*(?:\n[^'\n]+)*\n?'|\([^()]*\))/).getRegex(),Ev=G(/^(bull)([ \t][^\n]+?)?(?:\n|$)/).replace(/bull/g,Sa).getRegex(),Ps="address|article|aside|base|basefont|blockquote|body|caption|center|col|colgroup|dd|details|dialog|dir|div|dl|dt|fieldset|figcaption|figure|footer|form|frame|frameset|h[1-6]|head|header|hr|html|iframe|legend|li|link|main|menu|menuitem|meta|nav|noframes|ol|optgroup|option|p|param|search|section|summary|table|tbody|td|tfoot|th|thead|title|tr|track|ul",Ta=/<!--(?:-?>|[\s\S]*?(?:-->|$))/,Lv=G("^ {0,3}(?:<(script|pre|style|textarea)[\\s>][\\s\\S]*?(?:</\\1>[^\\n]*\\n+|$)|comment[^\\n]*(\\n+|$)|<\\?[\\s\\S]*?(?:\\?>\\n*|$)|<![A-Z][\\s\\S]*?(?:>\\n*|$)|<!\\[CDATA\\[[\\s\\S]*?(?:\\]\\]>\\n*|$)|</?(tag)(?: +|\\n|/?>)[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|<(?!script|pre|style|textarea)([a-z][\\w-]*)(?:attribute)*? */?>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|</(?!script|pre|style|textarea)[a-z][\\w-]*\\s*>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$))","i").replace("comment",Ta).replace("tag",Ps).replace("attribute",/ +[a-zA-Z:_][\w.:-]*(?: *= *"[^"\n]*"| *= *'[^'\n]*'| *= *[^\s"'=<>`]+)?/).getRegex(),rc=G(Aa).replace("hr",Dn).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("|lheading","").replace("|table","").replace("blockquote"," {0,3}>").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",Ps).getRegex(),Mv=G(/^( {0,3}> ?(paragraph|[^\n]*)(?:\n|$))+/).replace("paragraph",rc).getRegex(),_a={blockquote:Mv,code:kv,def:_v,fences:Sv,heading:Av,hr:Dn,html:Lv,lheading:oc,list:Ev,newline:wv,paragraph:rc,table:It,text:Tv},ar=G("^ *([^\\n ].*)\\n {0,3}((?:\\| *)?:?-+:? *(?:\\| *:?-+:? *)*(?:\\| *)?)(?:\\n((?:(?! *\\n|hr|heading|blockquote|code|fences|list|html).*(?:\\n|$))*)\\n*|$)").replace("hr",Dn).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("blockquote"," {0,3}>").replace("code","(?: {4}| {0,3}	)[^\\n]").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",Ps).getRegex(),Iv={..._a,lheading:Cv,table:ar,paragraph:G(Aa).replace("hr",Dn).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("|lheading","").replace("table",ar).replace("blockquote"," {0,3}>").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",Ps).getRegex()},Rv={..._a,html:G(`^ *(?:comment *(?:\\n|\\s*$)|<(tag)[\\s\\S]+?</\\1> *(?:\\n{2,}|\\s*$)|<tag(?:"[^"]*"|'[^']*'|\\s[^'"/>\\s]*)*?/?> *(?:\\n{2,}|\\s*$))`).replace("comment",Ta).replace(/tag/g,"(?!(?:a|em|strong|small|s|cite|q|dfn|abbr|data|time|code|var|samp|kbd|sub|sup|i|b|u|mark|ruby|rt|rp|bdi|bdo|span|br|wbr|ins|del|img)\\b)\\w+(?!:|[^\\w\\s@]*@)\\b").getRegex(),def:/^ *\[([^\]]+)\]: *<?([^\s>]+)>?(?: +(["(][^\n]+[")]))? *(?:\n+|$)/,heading:/^(#{1,6})(.*)(?:\n+|$)/,fences:It,lheading:/^(.+?)\n {0,3}(=+|-+) *(?:\n+|$)/,paragraph:G(Aa).replace("hr",Dn).replace("heading",` *#{1,6} *[^
]`).replace("lheading",oc).replace("|table","").replace("blockquote"," {0,3}>").replace("|fences","").replace("|list","").replace("|html","").replace("|tag","").getRegex()},Pv=/^\\([!"#$%&'()*+,\-./:;<=>?@\[\]\\^_`{|}~])/,Dv=/^(`+)([^`]|[^`][\s\S]*?[^`])\1(?!`)/,lc=/^( {2,}|\\)\n(?!\s*$)/,Nv=/^(`+|[^`])(?:(?= {2,}\n)|[\s\S]*?(?:(?=[\\<!\[`*_]|\b_|$)|[^ ](?= {2,}\n)))/,Ds=/[\p{P}\p{S}]/u,Ea=/[\s\p{P}\p{S}]/u,cc=/[^\s\p{P}\p{S}]/u,Fv=G(/^((?![*_])punctSpace)/,"u").replace(/punctSpace/g,Ea).getRegex(),dc=/(?!~)[\p{P}\p{S}]/u,Ov=/(?!~)[\s\p{P}\p{S}]/u,Bv=/(?:[^\s\p{P}\p{S}]|~)/u,uc=/(?![*_])[\p{P}\p{S}]/u,zv=/(?![*_])[\s\p{P}\p{S}]/u,Uv=/(?:[^\s\p{P}\p{S}]|[*_])/u,Hv=G(/link|precode-code|html/,"g").replace("link",/\[(?:[^\[\]`]|(?<a>`+)[^`]+\k<a>(?!`))*?\]\((?:\\[\s\S]|[^\\\(\)]|\((?:\\[\s\S]|[^\\\(\)])*\))*\)/).replace("precode-",$v?"(?<!`)()":"(^^|[^`])").replace("code",/(?<b>`+)[^`]+\k<b>(?!`)/).replace("html",/<(?! )[^<>]*?>/).getRegex(),gc=/^(?:\*+(?:((?!\*)punct)|[^\s*]))|^_+(?:((?!_)punct)|([^\s_]))/,jv=G(gc,"u").replace(/punct/g,Ds).getRegex(),Kv=G(gc,"u").replace(/punct/g,dc).getRegex(),pc="^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)",Wv=G(pc,"gu").replace(/notPunctSpace/g,cc).replace(/punctSpace/g,Ea).replace(/punct/g,Ds).getRegex(),qv=G(pc,"gu").replace(/notPunctSpace/g,Bv).replace(/punctSpace/g,Ov).replace(/punct/g,dc).getRegex(),Vv=G("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)punct(_+)(?=[\\s]|$)|notPunctSpace(_+)(?!_)(?=punctSpace|$)|(?!_)punctSpace(_+)(?=notPunctSpace)|[\\s](_+)(?!_)(?=punct)|(?!_)punct(_+)(?!_)(?=punct)","gu").replace(/notPunctSpace/g,cc).replace(/punctSpace/g,Ea).replace(/punct/g,Ds).getRegex(),Gv=G(/^~~?(?:((?!~)punct)|[^\s~])/,"u").replace(/punct/g,uc).getRegex(),Qv="^[^~]+(?=[^~])|(?!~)punct(~~?)(?=[\\s]|$)|notPunctSpace(~~?)(?!~)(?=punctSpace|$)|(?!~)punctSpace(~~?)(?=notPunctSpace)|[\\s](~~?)(?!~)(?=punct)|(?!~)punct(~~?)(?!~)(?=punct)|notPunctSpace(~~?)(?=notPunctSpace)",Yv=G(Qv,"gu").replace(/notPunctSpace/g,Uv).replace(/punctSpace/g,zv).replace(/punct/g,uc).getRegex(),Zv=G(/\\(punct)/,"gu").replace(/punct/g,Ds).getRegex(),Xv=G(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/).replace("scheme",/[a-zA-Z][a-zA-Z0-9+.-]{1,31}/).replace("email",/[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/).getRegex(),Jv=G(Ta).replace("(?:-->|$)","-->").getRegex(),em=G("^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>").replace("comment",Jv).replace("attribute",/\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/).getRegex(),ms=/(?:\[(?:\\[\s\S]|[^\[\]\\])*\]|\\[\s\S]|`+[^`]*?`+(?!`)|[^\[\]\\`])*?/,tm=G(/^!?\[(label)\]\(\s*(href)(?:(?:[ \t]*(?:\n[ \t]*)?)(title))?\s*\)/).replace("label",ms).replace("href",/<(?:\\.|[^\n<>\\])+>|[^ \t\n\x00-\x1f]*/).replace("title",/"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/).getRegex(),fc=G(/^!?\[(label)\]\[(ref)\]/).replace("label",ms).replace("ref",Ca).getRegex(),hc=G(/^!?\[(ref)\](?:\[\])?/).replace("ref",Ca).getRegex(),nm=G("reflink|nolink(?!\\()","g").replace("reflink",fc).replace("nolink",hc).getRegex(),or=/[hH][tT][tT][pP][sS]?|[fF][tT][pP]/,La={_backpedal:It,anyPunctuation:Zv,autolink:Xv,blockSkip:Hv,br:lc,code:Dv,del:It,delLDelim:It,delRDelim:It,emStrongLDelim:jv,emStrongRDelimAst:Wv,emStrongRDelimUnd:Vv,escape:Pv,link:tm,nolink:hc,punctuation:Fv,reflink:fc,reflinkSearch:nm,tag:em,text:Nv,url:It},sm={...La,link:G(/^!?\[(label)\]\((.*?)\)/).replace("label",ms).getRegex(),reflink:G(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label",ms).getRegex()},Ni={...La,emStrongRDelimAst:qv,emStrongLDelim:Kv,delLDelim:Gv,delRDelim:Yv,url:G(/^((?:protocol):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/).replace("protocol",or).replace("email",/[A-Za-z0-9._+-]+(@)[a-zA-Z0-9-_]+(?:\.[a-zA-Z0-9-_]*[a-zA-Z0-9])+(?![-_])/).getRegex(),_backpedal:/(?:[^?!.,:;*_'"~()&]+|\([^)]*\)|&(?![a-zA-Z0-9]+;$)|[?!.,:;*_'"~)]+(?!$))+/,del:/^(~~?)(?=[^\s~])((?:\\[\s\S]|[^\\])*?(?:\\[\s\S]|[^\s~\\]))\1(?=[^~]|$)/,text:G(/^([`~]+|[^`~])(?:(?= {2,}\n)|(?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)|[\s\S]*?(?:(?=[\\<!\[`*~_]|\b_|protocol:\/\/|www\.|$)|[^ ](?= {2,}\n)|[^a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-](?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)))/).replace("protocol",or).getRegex()},im={...Ni,br:G(lc).replace("{2,}","*").getRegex(),text:G(Ni.text).replace("\\b_","\\b_| {2,}\\n").replace(/\{2,\}/g,"*").getRegex()},Yn={normal:_a,gfm:Iv,pedantic:Rv},fn={normal:La,gfm:Ni,breaks:im,pedantic:sm},am={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"},rr=e=>am[e];function et(e,t){if(t){if($e.escapeTest.test(e))return e.replace($e.escapeReplace,rr)}else if($e.escapeTestNoEncode.test(e))return e.replace($e.escapeReplaceNoEncode,rr);return e}function lr(e){try{e=encodeURI(e).replace($e.percentDecode,"%")}catch{return null}return e}function cr(e,t){let n=e.replace($e.findPipe,(a,o,r)=>{let c=!1,d=o;for(;--d>=0&&r[d]==="\\";)c=!c;return c?"|":" |"}),s=n.split($e.splitPipe),i=0;if(s[0].trim()||s.shift(),s.length>0&&!s.at(-1)?.trim()&&s.pop(),t)if(s.length>t)s.splice(t);else for(;s.length<t;)s.push("");for(;i<s.length;i++)s[i]=s[i].trim().replace($e.slashPipe,"|");return s}function hn(e,t,n){let s=e.length;if(s===0)return"";let i=0;for(;i<s&&e.charAt(s-i-1)===t;)i++;return e.slice(0,s-i)}function om(e,t){if(e.indexOf(t[1])===-1)return-1;let n=0;for(let s=0;s<e.length;s++)if(e[s]==="\\")s++;else if(e[s]===t[0])n++;else if(e[s]===t[1]&&(n--,n<0))return s;return n>0?-2:-1}function rm(e,t=0){let n=t,s="";for(let i of e)if(i==="	"){let a=4-n%4;s+=" ".repeat(a),n+=a}else s+=i,n++;return s}function dr(e,t,n,s,i){let a=t.href,o=t.title||null,r=e[1].replace(i.other.outputLinkReplace,"$1");s.state.inLink=!0;let c={type:e[0].charAt(0)==="!"?"image":"link",raw:n,href:a,title:o,text:r,tokens:s.inlineTokens(r)};return s.state.inLink=!1,c}function lm(e,t,n){let s=e.match(n.other.indentCodeCompensation);if(s===null)return t;let i=s[1];return t.split(`
...
`)?"":`
`)+i.raw,o.text+=`
`+i.text,this.inlineQueue.pop(),this.inlineQueue.at(-1).src=o.text):n.push(i);continue}if(t){let o="Infinite loop on byte: "+t.charCodeAt(0);if(this.options.silent){console.error(o);break}else throw new Error(o)}}return this.state.top=!0,n}inline(t,n=[]){return this.inlineQueue.push({src:t,tokens:n}),n}inlineTokens(t,n=[]){let s=t,i=null;if(this.tokens.links){let c=Object.keys(this.tokens.links);if(c.length>0)for(;(i=this.tokenizer.rules.inline.reflinkSearch.exec(s))!=null;)c.includes(i[0].slice(i[0].lastIndexOf("[")+1,-1))&&(s=s.slice(0,i.index)+"["+"a".repeat(i[0].length-2)+"]"+s.slice(this.tokenizer.rules.inline.reflinkSearch.lastIndex))}for(;(i=this.tokenizer.rules.inline.anyPunctuation.exec(s))!=null;)s=s.slice(0,i.index)+"++"+s.slice(this.tokenizer.rules.inline.anyPunctuation.lastIndex);let a;for(;(i=this.tokenizer.rules.inline.blockSkip.exec(s))!=null;)a=i[2]?i[2].length:0,s=s.slice(0,i.index+a)+"["+"a".repeat(i[0].length-a-2)+"]"+s.slice(this.tokenizer.rules.inline.blockSkip.lastIndex);s=this.options.hooks?.emStrongMask?.call({lexer:this},s)??s;let o=!1,r="";for(;t;){o||(r=""),o=!1;let c;if(this.options.extensions?.inline?.some(g=>(c=g.call({lexer:this},t,n))?(t=t.substring(c.raw.length),n.push(c),!0):!1))continue;if(c=this.tokenizer.escape(t)){t=t.substring(c.raw.length),n.push(c);continue}if(c=this.tokenizer.tag(t)){t=t.substring(c.raw.length),n.push(c);continue}if(c=this.tokenizer.link(t)){t=t.substring(c.raw.length),n.push(c);continue}if(c=this.tokenizer.reflink(t,this.tokens.links)){t=t.substring(c.raw.length);let g=n.at(-1);c.type==="text"&&g?.type==="text"?(g.raw+=c.raw,g.text+=c.text):n.push(c);continue}if(c=this.tokenizer.emStrong(t,s,r)){t=t.substring(c.raw.length),n.push(c);continue}if(c=this.tokenizer.codespan(t)){t=t.substring(c.raw.length),n.push(c);continue}if(c=this.tokenizer.br(t)){t=t.substring(c.raw.length),n.push(c);continue}if(c=this.tokenizer.del(t,s,r)){t=t.substring(c.raw.length),n.push(c);continue}if(c=this.tokenizer.autolink(t)){t=t.substring(c.raw.length),n.push(c);continue}if(!this.state.inLink&&(c=this.tokenizer.url(t))){t=t.substring(c.raw.length),n.push(c);continue}let d=t;if(this.options.extensions?.startInline){let g=1/0,p=t.slice(1),h;this.options.extensions.startInline.forEach(u=>{h=u.call({lexer:this},p),typeof h=="number"&&h>=0&&(g=Math.min(g,h))}),g<1/0&&g>=0&&(d=t.substring(0,g+1))}if(c=this.tokenizer.inlineText(d)){t=t.substring(c.raw.length),c.raw.slice(-1)!=="_"&&(r=c.raw.slice(-1)),o=!0;let g=n.at(-1);g?.type==="text"?(g.raw+=c.raw,g.text+=c.text):n.push(c);continue}if(t){let g="Infinite loop on byte: "+t.charCodeAt(0);if(this.options.silent){console.error(g);break}else throw new Error(g)}}return n}},ys=class{options;parser;constructor(e){this.options=e||jt}space(e){return""}code({text:e,lang:t,escaped:n}){let s=(t||"").match($e.notSpaceStart)?.[0],i=e.replace($e.endingNewline,"")+`
...
`:""}*No output — tool completed successfully.*`;t(p)}:void 0,r=i&&(e.text?.length??0)<=Vm,c=i&&!r,d=i&&r,g=!i;return l`
    ...
    >
      ...
      ${g?l`
            ...
            `:v}
      ${c?l`<div class="chat-tool-card__preview mono">${Ym(e.text)}</div>`:v}
  ...
  `}const Na=["#e879f9","#38bdf8","#34d399","#fb923c","#a78bfa","#f472b6","#2dd4bf","#fbbf24","#818cf8","#4ade80"];function ib(e){let t=0;for(let n=0;n<e.length;n++)t=(t<<5)-t+e.charCodeAt(n)|0;return Math.abs(t)%Na.length}function ab(e){if(e.role!=="assistant"||!e.messages.length)return null;const t=e.messages[0]?.message;if(!t?.content)return null;const n=t.content;if(!Array.isArray(n))return null;for(const s of n)if(s.type==="text"&&typeof s.text=="string"){const i=s.text.match(/^\[([A-Za-z0-9_-]+(?:\s*→\s*[A-Za-z0-9_-]+)?)\]\s/);if(i)return i[1]}return null}function ob(e,t){const n=Na[t],s=e.split(/\s*→\s*/),a=(s[s.length-1]||e).charAt(0).toUpperCase();return l`<div class="chat-avatar injected" style="background: ${n}; color: #fff;">${a}</div>`}function rb(e,t){const n=Ra(e.role),s=t.assistantName??"Assistant",i=ab(e),a=i!==null,o=a?ib(i):-1,r=a?i:n==="user"?"You":n==="assistant"?s:n,c=a?"injected":n==="user"?"user":n==="assistant"?"assistant":"other",d=new Date(e.timestamp).toLocaleTimeString([],{hour:"numeric",minute:"2-digit"}),g=a?`--injected-color: ${Na[o]}`:"";return l`
    <div class="chat-group ${c}" style="${g}">
      ${a?ob(i,o):Fa(e.role,{name:s,avatar:t.assistantAvatar??null})}
      <div class="chat-group-messages">
  ...
  `}function Fa(e,t){const n=Ra(e),s=t?.name?.trim()||"Assistant",i=t?.avatar?.trim()||"",a=n==="user"?"U":n==="assistant"?s.charAt(0).toUpperCase()||"A":n==="tool"?"⚙":"?",o=n==="user"?"user":n==="assistant"?"assistant":n==="tool"?"tool":"other";return i&&n==="assistant"?lb(i)?l`<img
        class="chat-avatar ${o}"
        src="${i}"
        alt="${s}"
      />`:l`<div class="chat-avatar ${o}">${i}</div>`:l`<div class="chat-avatar ${o}">${a}</div>`}function lb(e){return/^https?:\/\//i.test(e)||/^data:image\//i.test(e)||e.startsWith("/")}function cb(e){return e.length===0?v:l`
  ...
  `}var ub=Object.defineProperty,gb=Object.getOwnPropertyDescriptor,Ns=(e,t,n,s)=>{for(var i=s>1?void 0:s?gb(t,n):t,a=e.length-1,o;a>=0;a--)(o=e[a])&&(i=(s?o(t,n,i):o(i))||i);return s&&i&&ub(t,n,i),i};let tn=class extends Xt{constructor(){super(...arguments),this.splitRatio=.6,this.minRatio=.4,this.maxRatio=.7,this.isDragging=!1,this.startX=0,this.startRatio=0,this.handleMouseDown=e=>{this.isDragging=!0,this.startX=e.clientX,this.startRatio=this.splitRatio,this.classList.add("dragging"),document.addEventListener("mousemove",this.handleMouseMove),document.addEventListener("mouseup",this.handleMouseUp),e.preventDefault()},this.handleMouseMove=e=>{if(!this.isDragging)return;const t=this.parentElement;if(!t)return;const n=t.getBoundingClientRect().width,i=(e.clientX-this.startX)/n;let a=this.startRatio+i;a=Math.max(this.minRatio,Math.min(this.maxRatio,a)),this.dispatchEvent(new CustomEvent("resize",{detail:{splitRatio:a},bubbles:!0,composed:!0}))},this.handleMouseUp=()=>{this.isDragging=!1,this.classList.remove("dragging"),document.removeEventListener("mousemove",this.handleMouseMove),document.removeEventListener("mouseup",this.handleMouseUp)}}render(){return v}connectedCallback(){super.connectedCallback(),this.addEventListener("mousedown",this.handleMouseDown)}disconnectedCallback(){super.disconnectedCallback(),this.removeEventListener("mousedown",this.handleMouseDown),document.removeEventListener("mousemove",this.handleMouseMove),document.removeEventListener("mouseup",this.handleMouseUp)}};tn.styles=Dc`
    :host {
      width: 4px;
      cursor: col-resize;
  ...
  `;Ns([ks({type:Number})],tn.prototype,"splitRatio",2);Ns([ks({type:Number})],tn.prototype,"minRatio",2);Ns([ks({type:Number})],tn.prototype,"maxRatio",2);tn=Ns([Fr("resizable-divider")],tn);const pb=5e3;function br(e){e.style.height="auto",e.style.height=`${e.scrollHeight}px`}function fb(e){return e?e.active?l`
  ...
  `}function bb(e){const t=e.connected,n=e.sending||e.stream!==null,s=!!(e.canAbort&&e.onAbort),a=e.sessions?.sessions?.find(f=>f.key===e.sessionKey)?.reasoningLevel??"off",o=e.showThinking&&a!=="off",r={name:e.assistantName,avatar:e.assistantAvatar??e.assistantAvatarUrl??null},c=(e.attachments?.length??0)>0,d=e.connected?c?"Add a message or paste more images...":"Message (↩ to send, Shift+↩ for line breaks, paste images)":"Connect to the gateway to start chatting…",g=e.splitRatio??.6,p=!!(e.sidebarOpen&&e.onCloseSidebar),h=xb(e),u=l`
    ...
    >
      ${e.loading?l`
            ...
            `:v}
      ${zl(h,f=>f.key,f=>f.kind==="divider"?l`
              <div class="chat-divider" role="separator" data-ts=${String(f.timestamp)}>
                <span class="chat-divider__line"></span>
                <span class="chat-divider__label">${f.label}</span>
                <span class="chat-divider__line"></span>
              </div>
            `:f.kind==="reading-indicator"?nb(r):f.kind==="stream"?sb(f.text,f.startedAt,e.onOpenSidebar,r):f.kind==="group"?rb(f,{onOpenSidebar:e.onOpenSidebar,showReasoning:o,assistantName:e.assistantName,assistantAvatar:r.avatar}):v)}
    </div>
  `;return l`
...
\`\`\``)}})}
        ...
        <div class="chat-compose__row">
          <label class="field chat-compose__field">
            ...
            <textarea
              ...
              @input=${f=>{const m=f.target;br(m),e.onDraftChange(m.value)}}
              @paste=${f=>vb(f,e)}
              placeholder=${d}
            ></textarea>
          </label>
          <div class="chat-compose__actions">
            ${s?l`<button
                  ...
                  ?disabled=${!e.connected}
                  @click=${e.onAbort}
                >Stop</button>`:l`${(e.agentsList?.agents??[]).length>0?l`<select
                        class="chat-agent-select"
                        ?disabled=${!e.connected||e.sending}
                        @change=${f=>{const m=f.target,w=m.value;m.value="",w==="__new__"?e.onNewSession():w&&e.onSelectAgent&&e.onSelectAgent(w)}}
                      >
                        <option value="" disabled selected>+ New ▾</option>
                        <option value="__new__">New chat (current agent)</option>
                        ${(e.agentsList?.agents??[]).map(f=>{const m=f.identity?.name||f.name||f.id;return l`<option value=${f.id}>New chat with ${m}</option>`})}
                      </select>`:l`<button
                        class="btn"
                        ?disabled=${!e.connected||e.sending}
  ...
  `},$r=[{key:"env",label:"Environment"},{key:"update",label:"Updates"},{key:"agents",label:"Agents"},{key:"auth",label:"Authentication"},{key:"channels",label:"Channels"},{key:"messages",label:"Messages"},{key:"commands",label:"Commands"},{key:"hooks",label:"Hooks"},{key:"skills",label:"Skills"},{key:"tools",label:"Tools"},{key:"gateway",label:"Gateway"},{key:"wizard",label:"Setup Wizard"}],wr="__all__";function kr(e){return zi[e]??zi.default}function $b(e,t){const n=wa[e];return n||{label:t?.title??nt(e),description:t?.description??""}}function wb(e){const{key:t,schema:n,uiHints:s}=e;if(!n||Ee(n)!=="object"||!n.properties)return[];const i=Object.entries(n.properties).map(([a,o])=>{const r=Ie([t,a],s),c=r?.label??o.title??nt(a),d=r?.help??o.description??"",g=r?.order??50;return{key:a,label:c,description:d,order:g}});return i.sort((a,o)=>a.order!==o.order?a.order-o.order:a.key.localeCompare(o.key)),i}function kb(e,t){if(!e||!t)return[];const n=[];function s(i,a,o){if(i===a)return;if(typeof i!=typeof a){n.push({path:o,from:i,to:a});return}if(typeof i!="object"||i===null||a===null){i!==a&&n.push({path:o,from:i,to:a});return}if(Array.isArray(i)&&Array.isArray(a)){JSON.stringify(i)!==JSON.stringify(a)&&n.push({path:o,from:i,to:a});return}const r=i,c=a,d=new Set([...Object.keys(r),...Object.keys(c)]);for(const g of d)s(r[g],c[g],o?`${o}.${g}`:g)}return s(e,t,""),n}function Sr(e,t=40){let n;try{n=JSON.stringify(e)??String(e)}catch{n=String(e)}return n.length<=t?n:n.slice(0,t-3)+"..."}function Sb(e){const t=e.valid==null?"unknown":e.valid?"valid":"invalid",n=Yl(e.schema),s=n.schema?n.unsupportedPaths.length>0:!1,i=n.schema?.properties??{},a=$r.filter(T=>T.key in i),o=new Set($r.map(T=>T.key)),r=Object.keys(i).filter(T=>!o.has(T)).map(T=>({key:T,label:T.charAt(0).toUpperCase()+T.slice(1)})),c=[...a,...r],d=e.activeSection&&n.schema&&Ee(n.schema)==="object"?n.schema.properties?.[e.activeSection]:void 0,g=e.activeSection?$b(e.activeSection,d):null,p=e.activeSection?wb({key:e.activeSection,schema:d,uiHints:e.uiHints}):[],h=e.formMode==="form"&&!!e.activeSection&&p.length>0,u=e.activeSubsection===wr,f=e.searchQuery||u?null:e.activeSubsection??p[0]?.key??null,m=e.formMode==="form"?kb(e.originalValue,e.formValue):[],w=e.formMode==="raw"&&e.raw!==e.originalRaw,S=e.formMode==="form"?m.length>0:w,C=!!e.formValue&&!e.loading&&!!n.schema,k=e.connected&&!e.saving&&S&&(e.formMode==="raw"?!0:C),A=e.connected&&!e.applying&&!e.updating&&S&&(e.formMode==="raw"?!0:C),E=e.connected&&!e.applying&&!e.updating;return l`
...
${JSON.stringify(e.issues,null,2)}</pre
  ...
  `}function Ib(e,t){const n=typeof e.sessionKey=="string"&&e.sessionKey.trim().length>0?`${Es("chat",t)}?session=${encodeURIComponent(e.sessionKey)}`:null;return l`
    <div class="list-item">
      <div class="list-main">
  ...
  `}function Pb(e){const t=Math.max(0,e),n=Math.floor(t/1e3);if(n<60)return`${n}s`;const s=Math.floor(n/60);return s<60?`${s}m`:`${Math.floor(s/60)}h`}function Tt(e,t){return t?l`<div class="exec-approval-meta-row"><span>${e}</span><span>${t}</span></div>`:v}function Db(e){const t=e.execApprovalQueue[0];if(!t)return v;const n=t.request,s=t.expiresAtMs-Date.now(),i=s>0?`expires in ${Pb(s)}`:"expired",a=e.execApprovalQueue.length;return l`
  ...
  `}function Nb(e){const{pendingGatewayUrl:t}=e;return t?l`
  ...
  `}function ay(e,t){const n=e.displayName?.trim()||e.deviceId,s=e.remoteIp?` · ${e.remoteIp}`:"",i=`roles: ${mi(e.roles)}`,a=`scopes: ${mi(e.scopes)}`,o=Array.isArray(e.tokens)?e.tokens:[];return l`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${n}</div>
  ...
  `}function oy(e,t,n){const s=t.revokedAtMs?"revoked":"active",i=`scopes: ${mi(t.scopes)}`,a=ee(t.rotatedAtMs??t.createdAtMs??t.lastUsedAtMs??null);return l`
  ...
  `}const fy=["","off","minimal","low","medium","high","xhigh"],hy=["","off","on"],vy=[{value:"",label:"inherit"},{value:"off",label:"off (explicit)"},{value:"on",label:"on"},{value:"full",label:"full"}],my=["","off","on","stream"];function by(e){if(!e)return"";const t=e.trim().toLowerCase();return t==="z.ai"||t==="z-ai"?"zai":t}function Cc(e){return by(e)==="zai"}function yy(e){return Cc(e)?hy:fy}function Er(e,t){return t?e.includes(t)?[...e]:[...e,t]:[...e]}function xy(e,t){return t?e.some(n=>n.value===t)?[...e]:[...e,{value:t,label:`${t} (custom)`}]:[...e]}function $y(e,t){return!t||!e||e==="off"?e:"on"}function wy(e,t){return e?t&&e==="on"?"low":e:null}function ky(e){const t=e.result?.sessions??[];return l`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">Sessions</div>
  ...
  `}function Sy(e,t,n,s,i){const a=e.updatedAt?ee(e.updatedAt):"n/a",o=e.thinkingLevel??"",r=Cc(e.modelProvider),c=$y(o,r),d=Er(yy(e.modelProvider),c),g=e.verboseLevel??"",p=xy(vy,g),h=e.reasoningLevel??"",u=Er(my,h),f=typeof e.displayName=="string"&&e.displayName.trim().length>0?e.displayName.trim():null,m=typeof e.label=="string"?e.label.trim():"",w=!!(f&&f!==e.key&&f!==m),S=e.kind!=="global",C=S?`${Es("chat",t)}?session=${encodeURIComponent(e.key)}`:null;return l`
    <div class="table-row">
  ...
  `}const Ty=/^data:/i,_y=/^https?:\/\//i;function Ey(e){const t=e.agentsList?.agents??[],s=qr(e.sessionKey)?.agentId??e.agentsList?.defaultId??"main",a=t.find(r=>r.id===s)?.identity,o=a?.avatarUrl??a?.avatar;if(o)return Ty.test(o)||_y.test(o)?o:a?.avatarUrl}function Ly(e){const t=e.presenceEntries.length,n=e.sessionsResult?.count??null,s=e.cronStatus?.nextWakeAtMs??null,i=e.connected?null:P("chat.disconnected"),a=e.tab==="chat",o=a&&(e.settings.chatFocusMode||e.onboarding),r=e.onboarding?!1:e.settings.chatShowThinking,c=Ey(e),d=e.chatAvatarUrl??c??null,g=e.configForm??e.configSnapshot?.config,p=nn(e.basePath??""),h=e.agentsSelectedId??e.agentsList?.defaultId??e.agentsList?.agents?.[0]?.id??null;return l`
    <div class="shell ${a?"shell--chat":""} ${o?"shell--chat-focus":""} ${e.settings.navCollapsed?"shell--nav-collapsed":""} ${e.onboarding?"shell--onboarding":""}">
      ...
      <aside class="nav ${e.settings.navCollapsed?"nav--collapsed":""}">
        ${Vu.map(u=>{const f=e.settings.navGroupsCollapsed[u.label]??!1,m=u.tabs.some(w=>w===e.tab);return l`
      ...
      <main class="content ${a?"content--chat":""}">
        <section class="content-header">
          <div>
            ${e.tab==="usage"?v:l`<div class="page-title">${ki(e.tab)}</div>`}
            ${e.tab==="usage"?v:l`<div class="page-sub">${Yu(e.tab)}</div>`}
          </div>
          <div class="page-meta">
