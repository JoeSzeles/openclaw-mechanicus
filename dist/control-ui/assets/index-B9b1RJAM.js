...
`)},Op=(e,t,n)=>{const s=e.trim();if(!s)return[];const i=s.length?s.split(/\s+/):[],o=i.length?i[i.length-1]:"",[a,r]=o.includes(":")?[o.slice(0,o.indexOf(":")),o.slice(o.indexOf(":")+1)]:["",""],c=a.toLowerCase(),d=r.toLowerCase(),g=C=>{const k=new Set;for(const A of C)A&&k.add(A);return Array.from(k)},p=g(t.map(C=>C.agentId)).slice(0,6),h=g(t.map(C=>C.channel)).slice(0,6),u=g([...t.map(C=>C.modelProvider),...t.map(C=>C.providerOverride),...n?.byProvider.map(C=>C.provider)??[]]).slice(0,6),f=g([...t.map(C=>C.model),...n?.byModel.map(C=>C.model)??[]]).slice(0,6),m=g(n?.tools.tools.map(C=>C.name)??[]).slice(0,6);if(!c)return[{label:"agent:",value:"agent:"},{label:"channel:",value:"channel:"},{label:"provider:",value:"provider:"},{label:"model:",value:"model:"},{label:"tool:",value:"tool:"},{label:"has:errors",value:"has:errors"},{label:"has:tools",value:"has:tools"},{label:"minTokens:",value:"minTokens:"},{label:"maxCost:",value:"maxCost:"}];const w=[],S=(C,k)=>{for(const A of k)(!d||A.toLowerCase().includes(d))&&w.push({label:`${C}:${A}`,value:`${C}:${A}`})};switch(c){case"agent":S("agent",p);break;case"channel":S("channel",h);break;case"provider":S("provider",u);break;case"model":S("model",f);break;case"tool":S("tool",m);break;case"has":["errors","tools","context","usage","model","provider"].forEach(C=>{(!d||C.includes(d))&&w.push({label:`has:${C}`,value:`has:${C}`})});break}return w},Bp=(e,t)=>{const n=e.trim();if(!n)return`${t} `;const s=n.split(/\s+/);return s[s.length-1]=t,`${s.join(" ")} `},Mt=e=>e.trim().toLowerCase(),zp=(e,t)=>{const n=e.trim();if(!n)return`${t} `;const s=n.split(/\s+/),i=s[s.length-1]??"",o=t.includes(":")?t.split(":")[0]:null,a=i.includes(":")?i.split(":")[0]:null;return i.endsWith(":")&&o&&a===o?(s[s.length-1]=t,`${s.join(" ")} `):s.includes(t)?`${s.join(" ")} `:`${s.join(" ")} ${t} `},Na=(e,t)=>{const s=e.trim().split(/\s+/).filter(Boolean).filter(i=>i!==t);return s.length?`${s.join(" ")} `:""},Oa=(e,t,n)=>{const s=Mt(t),o=[...yo(e).filter(a=>Mt(a.key??"")!==s).map(a=>a.raw),...n.map(a=>`${t}:${a}`)];return o.length?`${o.join(" ")} `:""};function pt(e,t){return t===0?0:e/t*100}function Up(e){const t=e.totalCost||0;return{input:{tokens:e.input,cost:e.inputCost||0,pct:pt(e.inputCost||0,t)},output:{tokens:e.output,cost:e.outputCost||0,pct:pt(e.outputCost||0,t)},cacheRead:{tokens:e.cacheRead,cost:e.cacheReadCost||0,pct:pt(e.cacheReadCost||0,t)},cacheWrite:{tokens:e.cacheWrite,cost:e.cacheWriteCost||0,pct:pt(e.cacheWriteCost||0,t)},totalCost:t}}function Hp(e,t,n,s,i,o,a,r){if(!(e.length>0||t.length>0||n.length>0))return v;const d=n.length===1?s.find(f=>f.key===n[0]):null,g=d?(d.label||d.key).slice(0,20)+((d.label||d.key).length>20?"…":""):n.length===1?n[0].slice(0,8)+"…":`${n.length} sessions`,p=d?d.label||d.key:n.length===1?n[0]:n.join(", "),h=e.length===1?e[0]:`${e.length} days`,u=t.length===1?`${t[0]}:00`:`${t.length} hours`;return l`
  ...
  `}function Kp(e,t){const n=Up(e),s=t==="tokens",i=e.totalTokens||1,o={output:pt(e.output,i),input:pt(e.input,i),cacheWrite:pt(e.cacheWrite,i),cacheRead:pt(e.cacheRead,i)};return l`
  ...
  `}function tf(e,t,n,s,i,o,a,r,c,d,g,p){if(t)return l`
    ...
    `;let h=e.points;if(a||r||c&&c.length>0){const U=a?new Date(a+"T00:00:00").getTime():0,ie=r?new Date(r+"T23:59:59").getTime():1/0;h=e.points.filter(ce=>{if(ce.timestamp<U||ce.timestamp>ie)return!1;if(c&&c.length>0){const he=new Date(ce.timestamp),_e=`${he.getFullYear()}-${String(he.getMonth()+1).padStart(2,"0")}-${String(he.getDate()).padStart(2,"0")}`;return c.includes(_e)}return!0})}if(h.length<2)return l`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">No data in range</div>
      </div>
    `;let u=0,f=0,m=0,w=0,S=0,C=0;h=h.map(U=>(u+=U.totalTokens,f+=U.cost,m+=U.output,w+=U.input,S+=U.cacheRead,C+=U.cacheWrite,{...U,cumulativeTokens:u,cumulativeCost:f}));const k=d!=null&&g!=null,A=k?Math.min(d,g):0,E=k?Math.max(d,g):1/0;let T=0,I=h.length;if(k){T=h.findIndex(ie=>ie.timestamp>=A),T===-1&&(T=h.length);const U=h.findIndex(ie=>ie.timestamp>E);I=U===-1?h.length:U}const H=k?h.slice(T,I):h;let Z=0,te=0,R=0,q=0;for(const U of H)Z+=U.output,te+=U.input,R+=U.cacheRead,q+=U.cacheWrite;const ne=400,re=100,_={top:8,right:4,bottom:14,left:30},j=ne-_.left-_.right,Q=re-_.top-_.bottom,le=n==="cumulative",fe=n==="per-turn"&&i==="by-type",M=Z+te+R+q,D=h.map(U=>le?U.cumulativeTokens:fe?U.input+U.output+U.cacheRead+U.cacheWrite:U.totalTokens),F=Math.max(...D,1),K=j/h.length,de=Math.min(Gp,Math.max(1,K*Vp)),J=K-de,se=_.left+T*(de+J),V=I>=h.length?_.left+(h.length-1)*(de+J)+de:_.left+(I-1)*(de+J)+de;return l`
    <div class="session-timeseries-compact">
      ...
      <div class="timeseries-chart-wrapper" style="position: relative; cursor: crosshair;">
        ...
        >
          ...
          `:v}
          <!-- Bars -->
          ${h.map((U,ie)=>{const ce=D[ie],he=_.left+ie*(de+J),_e=ce/F*Q,Ve=_.top+Q-_e,ve=[new Date(U.timestamp).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),`${B(ce)} tokens`];fe&&(ve.push(`Out ${B(U.output)}`),ve.push(`In ${B(U.input)}`),ve.push(`CW ${B(U.cacheWrite)}`),ve.push(`CR ${B(U.cacheRead)}`));const ze=ve.join(" · "),Ge=k&&(ie<T||ie>=I);if(!fe)return St`<rect x="${he}" y="${Ve}" width="${de}" height="${_e}" class="ts-bar${Ge?" dimmed":""}" rx="1"><title>${ze}</title></rect>`;const Qe=[{value:U.output,cls:"output"},{value:U.input,cls:"input"},{value:U.cacheWrite,cls:"cache-write"},{value:U.cacheRead,cls:"cache-read"}];let Ye=_.top+Q;const at=Ge?" dimmed":"";return St`
              ${Qe.map(rt=>{if(rt.value<=0||ce<=0)return v;const $t=_e*(rt.value/ce);return Ye-=$t,St`<rect x="${he}" y="${Ye}" width="${de}" height="${$t}" class="ts-bar ${rt.cls}${at}" rx="1"><title>${ze}</title></rect>`})}
            `})}
        ...
        ${(()=>{const U=`${(se/ne*100).toFixed(1)}%`,ie=`${(V/ne*100).toFixed(1)}%`,ce=he=>_e=>{if(!p)return;_e.preventDefault(),_e.stopPropagation();const ot=_e.currentTarget.closest(".timeseries-chart-wrapper")?.querySelector("svg");if(!ot)return;const ve=ot.getBoundingClientRect(),ze=ve.width,Ge=_.left/ne*ze,Ye=(ne-_.right)/ne*ze-Ge,at=Ue=>{const Se=Math.max(0,Math.min(1,(Ue-ve.left-Ge)/Ye));return Math.min(Math.floor(Se*h.length),h.length-1)},rt=he==="left"?se:V,$t=ve.left+rt/ne*ze,Bs=_e.clientX-$t;document.body.style.cursor="col-resize";const Kt=Ue=>{const Se=Ue.clientX-Bs,an=at(Se),Wt=h[an];if(Wt)if(he==="left"){const ct=g??h[h.length-1].timestamp;p(Math.min(Wt.timestamp,ct),ct)}else{const ct=d??h[0].timestamp;p(ct,Math.max(Wt.timestamp,ct))}},lt=()=>{document.body.style.cursor="",document.removeEventListener("mousemove",Kt),document.removeEventListener("mouseup",lt)};document.addEventListener("mousemove",Kt),document.addEventListener("mouseup",lt)};return l`
...
`);function cf(e){if(e.loading&&!e.totals)return l`
      ...
      >
        ...
        <div class="usage-filter-popover">
          ...
          <div class="usage-filter-options">
            ${F.map(V=>{const U=de.has(Mt(V));return l`
                <label class="usage-filter-option">
                  <input
                    type="checkbox"
                    .checked=${U}
                    @change=${ie=>{const ce=ie.target,he=`${M}:${V}`;e.onQueryDraftChange(ce.checked?zp(e.queryDraft,he):Na(e.queryDraft,he))}}
                  />
                  <span>${V}</span>
                </label>
              `})}
  ...
  `}function Bh(e,t){const n=t.snapshot,s=n?.channels;if(!n||!s)return!1;const i=s[e],o=typeof i?.configured=="boolean"&&i.configured,a=typeof i?.running=="boolean"&&i.running,r=typeof i?.connected=="boolean"&&i.connected,d=(n.channelAccounts?.[e]??[]).some(g=>g.configured||g.running||g.connected);return o||a||r||d}function zh(e,t){return t?.[e]?.length??0}function nc(e,t){const n=zh(e,t);return n<2?v:l`<div class="account-count">Accounts (${n})</div>`}function Uh(e){const{props:t,signal:n,accountCountLabel:s}=e;return l`
    <div class="card">
...
`:""}*No output — tool completed successfully.*`;t(p)}:void 0,r=i&&(e.text?.length??0)<=Xm,c=i&&!r,d=i&&r,g=!i;return l`
  ...
  `}function Uo(e,t){const n=No(e),s=t?.name?.trim()||"Assistant",i=t?.avatar?.trim()||"",o=n==="user"?"U":n==="assistant"?s.charAt(0).toUpperCase()||"A":n==="tool"?"⚙":"?",a=n==="user"?"user":n==="assistant"?"assistant":n==="tool"?"tool":"other";return i&&n==="assistant"?fb(i)?l`<img
        class="chat-avatar ${a}"
        src="${i}"
        alt="${s}"
  ...
  `;Os([Ss({type:Number})],nn.prototype,"splitRatio",2);Os([Ss({type:Number})],nn.prototype,"minRatio",2);Os([Ss({type:Number})],nn.prototype,"maxRatio",2);nn=Os([Ur("resizable-divider")],nn);const yb=5e3;function wr(e){e.style.height="auto",e.style.height=`${e.scrollHeight}px`}function xb(e){return e?e.active?l`
...
${JSON.stringify(e.issues,null,2)}</pre
  ...
  `}function zb(e){const t=Math.max(0,e),n=Math.floor(t/1e3);if(n<60)return`${n}s`;const s=Math.floor(n/60);return s<60?`${s}m`:`${Math.floor(s/60)}h`}function _t(e,t){return t?l`<div class="exec-approval-meta-row"><span>${e}</span><span>${t}</span></div>`:v}function Ub(e){const t=e.execApprovalQueue[0];if(!t)return v;const n=t.request,s=t.expiresAtMs-Date.now(),i=s>0?`expires in ${zb(s)}`:"expired",o=e.execApprovalQueue.length;return l`
