...
`);function nf(e){if(e.loading&&!e.totals)return l`
      ...
      >
        ...
        <div class="usage-filter-popover">
          <div class="usage-filter-actions">
            <button
              class="btn btn-sm"
              @click=${V=>{V.preventDefault(),V.stopPropagation(),e.onQueryDraftChange(Ro(e.queryDraft,M,N))}}
              ?disabled=${X}
            >
              Select All
            </button>
            <button
              class="btn btn-sm"
              @click=${V=>{V.preventDefault(),V.stopPropagation(),e.onQueryDraftChange(Ro(e.queryDraft,M,[]))}}
              ?disabled=${se===0}
            >
              Clear
            </button>
          ...
          <div class="usage-filter-options">
            ${N.map(V=>{const U=ce.has(Et(V));return l`
                <label class="usage-filter-option">
                  <input
                    type="checkbox"
                    .checked=${U}
                    @change=${ie=>{const le=ie.target,fe=`${M}:${V}`;e.onQueryDraftChange(le.checked?Pp(e.queryDraft,fe):Io(e.queryDraft,fe))}}
                  />
                  <span>${V}</span>
                </label>
              `})}
                ...
                <div class="usage-query-chips">
                  ${h.map(M=>{const D=M.raw;return l`
                      <span class="usage-query-chip">
                        ...
                        <button
                          title="Remove filter"
                          @click=${()=>e.onQueryDraftChange(Io(e.queryDraft,D))}
                        >
                          ×
                        </button>
                      </span>
    ...
    <div class="chat-controls">
      <label class="field chat-controls__session">
        <select
          ...
          @change=${d=>{const g=d.target.value;e.sessionKey=g,e.chatMessage="",e.chatStream=null,e.chatStreamStartedAt=null,e.chatRunId=null,e.resetToolStream(),e.resetChatScroll(),e.applySettings({...e.settings,sessionKey:g,lastActiveSessionKey:g}),e.loadAssistantIdentity(),ug(e,g),Ln(e)}}
        >
  ...
  `}function sh(e){const t=Is(e.configForm,e.agentId),n=t.entry?.tools??{},s=t.globalTools??{},i=n.profile??s.profile??"full",a=n.profile?"agent override":s.profile?"global default":"default",o=Array.isArray(n.allow)&&n.allow.length>0,r=Array.isArray(s.allow)&&s.allow.length>0,c=!!e.configForm&&!e.configLoading&&!e.configSaving&&!o,d=o?[]:Array.isArray(n.alsoAllow)?n.alsoAllow:[],g=o?[]:Array.isArray(n.deny)?n.deny:[],p=o?{allow:n.allow??[],deny:n.deny??[]}:Wf(i)??void 0,h=zo.flatMap(S=>S.tools.map(C=>C.id)),u=S=>{const C=Kf(S,p),k=Ko(S,d),A=Ko(S,g);return{allowed:(C||k)&&!A,baseAllowed:C,denied:A}},f=h.filter(S=>u(S).allowed).length,v=(S,C)=>{const k=new Set(d.map(I=>je(I)).filter(I=>I.length>0)),A=new Set(g.map(I=>je(I)).filter(I=>I.length>0)),E=u(S).baseAllowed,T=je(S);C?(A.delete(T),E||k.add(T)):(k.delete(T),A.add(T)),e.onOverridesChange(e.agentId,[...k],[...A])},w=S=>{const C=new Set(d.map(A=>je(A)).filter(A=>A.length>0)),k=new Set(g.map(A=>je(A)).filter(A=>A.length>0));for(const A of h){const E=u(A).baseAllowed,T=je(A);S?(k.delete(T),E||C.add(T)):(C.delete(T),k.add(T))}e.onOverridesChange(e.agentId,[...C],[...k])};return l`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
    ...
    <div class="agents-layout">
      ...
      <section class="agents-main">
        ${i?l`
                ...
                ${e.activePanel==="channels"?Jf({context:Uo(i,e.configForm,e.agentFilesList,n,e.agentIdentityById[i.id]??null),configForm:e.configForm,snapshot:e.channelsSnapshot,loading:e.channelsLoading,error:e.channelsError,lastSuccess:e.channelsLastSuccess,onRefresh:e.onChannelsRefresh}):m}
                ${e.activePanel==="cron"?eh({context:Uo(i,e.configForm,e.agentFilesList,n,e.agentIdentityById[i.id]??null),agentId:i.id,jobs:e.cronJobs,status:e.cronStatus,loading:e.cronLoading,error:e.cronError,onRefresh:e.onCronRefresh}):m}
              `:l`
                <div class="card">
  ...
  `}function dh(e){const{agent:t,configForm:n,agentFilesList:s,agentIdentity:i,agentIdentityLoading:a,agentIdentityError:o,configLoading:r,configSaving:c,configDirty:d,onConfigReload:g,onConfigSave:p,onModelChange:h,onModelFallbacksChange:u}=e,f=Is(n,t.id),w=(s&&s.agentId===t.id?s.workspace:null)||f.entry?.workspace||f.defaults?.workspace||"default",S=f.entry?.model?yn(f.entry?.model):yn(f.defaults?.model),C=yn(f.defaults?.model),k=jo(f.entry?.model)||(S!=="-"?Ho(S):null),A=jo(f.defaults?.model)||(C!=="-"?Ho(C):null),E=k??A??null,T=Bf(f.entry?.model),I=T?T.join(", "):"",H=i?.name?.trim()||t.identity?.name?.trim()||t.name?.trim()||f.entry?.name||"-",te=Ms(t,i)||"-",R=Array.isArray(f.entry?.skills)?f.entry?.skills:null,q=R?.length??null,ne=a?"Loading…":o?"Unavailable":"",oe=!!(e.defaultId&&t.id===e.defaultId);return l`
    <section class="card">
  ...
  `};function mt(e){const{schema:t,value:n,path:s,hints:i,unsupported:a,disabled:o,onPatch:r}=e,c=e.showLabel??!0,d=Ee(t),g=Ie(s,i),p=g?.label??t.title??tt(String(s.at(-1))),h=g?.help??t.description,u=Vi(s);if(a.has(u))return l`<div class="cfg-field cfg-field--error">
      ...
      `}if(C&&S.length>5)return qo({...e,options:S,value:n??t.default});const k=new Set(v.map(E=>Ee(E)).filter(Boolean)),A=new Set([...k].map(E=>E==="integer"?"number":E));if([...A].every(E=>["string","number","boolean"].includes(E))){const E=A.has("string"),T=A.has("number");if(A.has("boolean")&&A.size===1)return mt({...e,schema:{...t,type:"boolean",anyOf:void 0,oneOf:void 0}});if(E||T)return Wo({...e,inputType:T&&!E?"number":"text"})}}if(t.enum){const f=t.enum;if(f.length<=5){const v=n??t.default;return l`
        <div class="cfg-field">
          ${c?l`<label class="cfg-field__label">${p}</label>`:m}
          ${h?l`<div class="cfg-field__help">${h}</div>`:m}
          <div class="cfg-segmented">
      ...
      `}return qo({...e,options:f,value:n??t.default})}if(d==="object")return hh(e);if(d==="array")return mh(e);if(d==="boolean"){const f=typeof n=="boolean"?n:typeof t.default=="boolean"?t.default:!1;return l`
    ...
    `}return d==="number"||d==="integer"?fh(e):d==="string"?Wo({...e,inputType:"text"}):l`
    <div class="cfg-field cfg-field--error">
      <div class="cfg-field__label">${p}</div>
      <div class="cfg-field__error">Unsupported type: ${d}. Use Raw mode.</div>
    </div>
  `}function Wo(e){const{schema:t,value:n,path:s,hints:i,disabled:a,onPatch:o,inputType:r}=e,c=e.showLabel??!0,d=Ie(s,i),g=d?.label??t.title??tt(String(s.at(-1))),p=d?.help??t.description,h=(d?.sensitive??!1)&&!/^\$\{[^}]*\}$/.test(String(n??"").trim()),u=d?.placeholder??(h?"••••":t.default!==void 0?`Default: ${String(t.default)}`:""),f=n??"";return l`
    <div class="cfg-field">
  ...
  `}function qo(e){const{schema:t,value:n,path:s,hints:i,disabled:a,options:o,onPatch:r}=e,c=e.showLabel??!0,d=Ie(s,i),g=d?.label??t.title??tt(String(s.at(-1))),p=d?.help??t.description,h=n??t.default,u=o.findIndex(v=>v===h||String(v)===String(h)),f="__unset__";return l`
    <div class="cfg-field">
      ${c?l`<label class="cfg-field__label">${g}</label>`:m}
      ${p?l`<div class="cfg-field__help">${p}</div>`:m}
      <select
  ...
  `}function mh(e){const{schema:t,value:n,path:s,hints:i,unsupported:a,disabled:o,onPatch:r}=e,c=e.showLabel??!0,d=Ie(s,i),g=d?.label??t.title??tt(String(s.at(-1))),p=d?.help??t.description,h=Array.isArray(t.items)?t.items[0]:t.items;if(!h)return l`
      <div class="cfg-field cfg-field--error">
      ...
      ${g?(()=>{const{sectionKey:p,subsectionKey:h,schema:u}=g,f=Ie([p,h],e.uiHints),v=f?.label??u.title??tt(h),w=f?.help??u.description??"",S=n[p],C=S&&typeof S=="object"?S[h]:void 0,k=`config-section-${p}-${h}`;return l`
              <section class="config-section-card" id=${k}>
                <div class="config-section-card__header">
                  <span class="config-section-card__icon">${Go(p)}</span>
...
${e.snapshot?JSON.stringify(e.snapshot,null,2):"No snapshot yet."}
  ...
  `}const wn=(e,t)=>{const n=e._$AN;if(n===void 0)return!1;for(const s of n)s._$AO?.(t,!1),wn(s,t);return!0},hs=e=>{let t,n;do{if((t=e._$AM)===void 0)break;n=t._$AN,n.delete(e),e=t}while(n?.size===0)},Jl=e=>{for(let t;t=e._$AM;e=t){let n=t._$AN;if(n===void 0)t._$AN=n=new Set;else if(n.has(e))break;n.add(e),Zh(t)}};function Qh(e){this._$AN!==void 0?(hs(this),this._$AM=e,Jl(this)):this._$AM=e}function Yh(e,t=!1,n=0){const s=this._$AH,i=this._$AN;if(i!==void 0&&i.size!==0)if(t)if(Array.isArray(s))for(let a=n;a<s.length;a++)wn(s[a],!1),hs(s[a]);else s!=null&&(wn(s,!1),hs(s));else wn(this,e)}const Zh=e=>{e.type==va.CHILD&&(e._$AP??=Yh,e._$AQ??=Qh)};class Xh extends ya{constructor(){super(...arguments),this._$AN=void 0}_$AT(t,n,s){super._$AT(t,n,s),Jl(this),this.isConnected=t._$AU}_$AO(t,n=!0){t!==this.isConnected&&(this.isConnected=t,t?this.reconnected?.():this.disconnected?.()),n&&(wn(this,t),hs(this))}setValue(t){if(of(this._$Ct))this._$Ct._$AI(t,this);else{const n=[...this._$Ct._$AH];n[this._$Ci]=t,this._$Ct._$AI(n,this,0)}}disconnected(){}reconnected(){}}const oi=new WeakMap,Jh=ba(class extends Xh{render(e){return m}update(e,[t]){const n=t!==this.G;return n&&this.G!==void 0&&this.rt(void 0),(n||this.lt!==this.ct)&&(this.G=t,this.ht=e.options?.host,this.rt(this.ct=e.element)),m}rt(e){if(this.isConnected||(e=void 0),typeof this.G=="function"){const t=this.ht??globalThis;let n=oi.get(t);n===void 0&&(n=new WeakMap,oi.set(t,n)),n.get(this.G)!==void 0&&this.G.call(this.ht,void 0),n.set(this.G,e),e!==void 0&&this.G.call(this.ht,e)}else this.G.value=e}get lt(){return typeof this.G=="function"?oi.get(this.ht??globalThis)?.get(this.G):this.G?.value}disconnected(){this.lt===this.ct&&this.rt(void 0)}reconnected(){this.rt(this.ct)}});class Ei extends ya{constructor(t){if(super(t),this.it=m,t.type!==va.CHILD)throw Error(this.constructor.directiveName+"() can only be used in child bindings")}render(t){if(t===m||t==null)return this._t=void 0,this.it=t;if(t===ft)return t;if(typeof t!="string")throw Error(this.constructor.directiveName+"() called with a non-string value");if(t===this.it)return this._t;this.it=t;const n=[t];return n.raw=n,this._t={_$litType$:this.constructor.resultType,strings:n,values:[]}}}Ei.directiveName="unsafeHTML",Ei.resultType=1;const Li=ba(Ei);const{entries:ec,setPrototypeOf:Yo,isFrozen:em,getPrototypeOf:tm,getOwnPropertyDescriptor:nm}=Object;let{freeze:we,seal:Re,create:Mi}=Object,{apply:Ii,construct:Ri}=typeof Reflect<"u"&&Reflect;we||(we=function(t){return t});Re||(Re=function(t){return t});Ii||(Ii=function(t,n){for(var s=arguments.length,i=new Array(s>2?s-2:0),a=2;a<s;a++)i[a-2]=arguments[a];return t.apply(n,i)});Ri||(Ri=function(t){for(var n=arguments.length,s=new Array(n>1?n-1:0),i=1;i<n;i++)s[i-1]=arguments[i];return new t(...s)});const Gn=ke(Array.prototype.forEach),sm=ke(Array.prototype.lastIndexOf),Zo=ke(Array.prototype.pop),ln=ke(Array.prototype.push),im=ke(Array.prototype.splice),os=ke(String.prototype.toLowerCase),ri=ke(String.prototype.toString),li=ke(String.prototype.match),cn=ke(String.prototype.replace),am=ke(String.prototype.indexOf),om=ke(String.prototype.trim),Pe=ke(Object.prototype.hasOwnProperty),xe=ke(RegExp.prototype.test),dn=rm(TypeError);function ke(e){return function(t){t instanceof RegExp&&(t.lastIndex=0);for(var n=arguments.length,s=new Array(n>1?n-1:0),i=1;i<n;i++)s[i-1]=arguments[i];return Ii(e,t,s)}}function rm(e){return function(){for(var t=arguments.length,n=new Array(t),s=0;s<t;s++)n[s]=arguments[s];return Ri(e,n)}}function W(e,t){let n=arguments.length>2&&arguments[2]!==void 0?arguments[2]:os;Yo&&Yo(e,null);let s=t.length;for(;s--;){let i=t[s];if(typeof i=="string"){const a=n(i);a!==i&&(em(t)||(t[s]=a),i=a)}e[i]=!0}return e}function lm(e){for(let t=0;t<e.length;t++)Pe(e,t)||(e[t]=null);return e}function He(e){const t=Mi(null);for(const[n,s]of ec(e))Pe(e,n)&&(Array.isArray(s)?t[n]=lm(s):s&&typeof s=="object"&&s.constructor===Object?t[n]=He(s):t[n]=s);return t}function un(e,t){for(;e!==null;){const s=nm(e,t);if(s){if(s.get)return ke(s.get);if(typeof s.value=="function")return ke(s.value)}e=tm(e)}function n(){return null}return n}const Xo=we(["a","abbr","acronym","address","area","article","aside","audio","b","bdi","bdo","big","blink","blockquote","body","br","button","canvas","caption","center","cite","code","col","colgroup","content","data","datalist","dd","decorator","del","details","dfn","dialog","dir","div","dl","dt","element","em","fieldset","figcaption","figure","font","footer","form","h1","h2","h3","h4","h5","h6","head","header","hgroup","hr","html","i","img","input","ins","kbd","label","legend","li","main","map","mark","marquee","menu","menuitem","meter","nav","nobr","ol","optgroup","option","output","p","picture","pre","progress","q","rp","rt","ruby","s","samp","search","section","select","shadow","slot","small","source","spacer","span","strike","strong","style","sub","summary","sup","table","tbody","td","template","textarea","tfoot","th","thead","time","tr","track","tt","u","ul","var","video","wbr"]),ci=we(["svg","a","altglyph","altglyphdef","altglyphitem","animatecolor","animatemotion","animatetransform","circle","clippath","defs","desc","ellipse","enterkeyhint","exportparts","filter","font","g","glyph","glyphref","hkern","image","inputmode","line","lineargradient","marker","mask","metadata","mpath","part","path","pattern","polygon","polyline","radialgradient","rect","stop","style","switch","symbol","text","textpath","title","tref","tspan","view","vkern"]),di=we(["feBlend","feColorMatrix","feComponentTransfer","feComposite","feConvolveMatrix","feDiffuseLighting","feDisplacementMap","feDistantLight","feDropShadow","feFlood","feFuncA","feFuncB","feFuncG","feFuncR","feGaussianBlur","feImage","feMerge","feMergeNode","feMorphology","feOffset","fePointLight","feSpecularLighting","feSpotLight","feTile","feTurbulence"]),cm=we(["animate","color-profile","cursor","discard","font-face","font-face-format","font-face-name","font-face-src","font-face-uri","foreignobject","hatch","hatchpath","mesh","meshgradient","meshpatch","meshrow","missing-glyph","script","set","solidcolor","unknown","use"]),ui=we(["math","menclose","merror","mfenced","mfrac","mglyph","mi","mlabeledtr","mmultiscripts","mn","mo","mover","mpadded","mphantom","mroot","mrow","ms","mspace","msqrt","mstyle","msub","msup","msubsup","mtable","mtd","mtext","mtr","munder","munderover","mprescripts"]),dm=we(["maction","maligngroup","malignmark","mlongdiv","mscarries","mscarry","msgroup","mstack","msline","msrow","semantics","annotation","annotation-xml","mprescripts","none"]),Jo=we(["#text"]),er=we(["accept","action","align","alt","autocapitalize","autocomplete","autopictureinpicture","autoplay","background","bgcolor","border","capture","cellpadding","cellspacing","checked","cite","class","clear","color","cols","colspan","controls","controlslist","coords","crossorigin","datetime","decoding","default","dir","disabled","disablepictureinpicture","disableremoteplayback","download","draggable","enctype","enterkeyhint","exportparts","face","for","headers","height","hidden","high","href","hreflang","id","inert","inputmode","integrity","ismap","kind","label","lang","list","loading","loop","low","max","maxlength","media","method","min","minlength","multiple","muted","name","nonce","noshade","novalidate","nowrap","open","optimum","part","pattern","placeholder","playsinline","popover","popovertarget","popovertargetaction","poster","preload","pubdate","radiogroup","readonly","rel","required","rev","reversed","role","rows","rowspan","spellcheck","scope","selected","shape","size","sizes","slot","span","srclang","start","src","srcset","step","style","summary","tabindex","title","translate","type","usemap","valign","value","width","wrap","xmlns","slot"]),gi=we(["accent-height","accumulate","additive","alignment-baseline","amplitude","ascent","attributename","attributetype","azimuth","basefrequency","baseline-shift","begin","bias","by","class","clip","clippathunits","clip-path","clip-rule","color","color-interpolation","color-interpolation-filters","color-profile","color-rendering","cx","cy","d","dx","dy","diffuseconstant","direction","display","divisor","dur","edgemode","elevation","end","exponent","fill","fill-opacity","fill-rule","filter","filterunits","flood-color","flood-opacity","font-family","font-size","font-size-adjust","font-stretch","font-style","font-variant","font-weight","fx","fy","g1","g2","glyph-name","glyphref","gradientunits","gradienttransform","height","href","id","image-rendering","in","in2","intercept","k","k1","k2","k3","k4","kerning","keypoints","keysplines","keytimes","lang","lengthadjust","letter-spacing","kernelmatrix","kernelunitlength","lighting-color","local","marker-end","marker-mid","marker-start","markerheight","markerunits","markerwidth","maskcontentunits","maskunits","max","mask","mask-type","media","method","mode","min","name","numoctaves","offset","operator","opacity","order","orient","orientation","origin","overflow","paint-order","path","pathlength","patterncontentunits","patterntransform","patternunits","points","preservealpha","preserveaspectratio","primitiveunits","r","rx","ry","radius","refx","refy","repeatcount","repeatdur","restart","result","rotate","scale","seed","shape-rendering","slope","specularconstant","specularexponent","spreadmethod","startoffset","stddeviation","stitchtiles","stop-color","stop-opacity","stroke-dasharray","stroke-dashoffset","stroke-linecap","stroke-linejoin","stroke-miterlimit","stroke-opacity","stroke","stroke-width","style","surfacescale","systemlanguage","tabindex","tablevalues","targetx","targety","transform","transform-origin","text-anchor","text-decoration","text-rendering","textlength","type","u1","u2","unicode","values","viewbox","visibility","version","vert-adv-y","vert-origin-x","vert-origin-y","width","word-spacing","wrap","writing-mode","xchannelselector","ychannelselector","x","x1","x2","xmlns","y","y1","y2","z","zoomandpan"]),tr=we(["accent","accentunder","align","bevelled","close","columnsalign","columnlines","columnspan","denomalign","depth","dir","display","displaystyle","encoding","fence","frame","height","href","id","largeop","length","linethickness","lspace","lquote","mathbackground","mathcolor","mathsize","mathvariant","maxsize","minsize","movablelimits","notation","numalign","open","rowalign","rowlines","rowspacing","rowspan","rspace","rquote","scriptlevel","scriptminsize","scriptsizemultiplier","selection","separator","separators","stretchy","subscriptshift","supscriptshift","symmetric","voffset","width","xmlns"]),Qn=we(["xlink:href","xml:id","xlink:title","xml:space","xmlns:xlink"]),um=Re(/\{\{[\w\W]*|[\w\W]*\}\}/gm),gm=Re(/<%[\w\W]*|[\w\W]*%>/gm),pm=Re(/\$\{[\w\W]*/gm),fm=Re(/^data-[\-\w.\u00B7-\uFFFF]+$/),hm=Re(/^aria-[\-\w]+$/),tc=Re(/^(?:(?:(?:f|ht)tps?|mailto|tel|callto|sms|cid|xmpp|matrix):|[^a-z]|[a-z+.\-]+(?:[^a-z+.\-:]|$))/i),mm=Re(/^(?:\w+script|data):/i),vm=Re(/[\u0000-\u0020\u00A0\u1680\u180E\u2000-\u2029\u205F\u3000]/g),nc=Re(/^html$/i),bm=Re(/^[a-z][.\w]*(-[.\w]+)+$/i);var nr=Object.freeze({__proto__:null,ARIA_ATTR:hm,ATTR_WHITESPACE:vm,CUSTOM_ELEMENT:bm,DATA_ATTR:fm,DOCTYPE_NAME:nc,ERB_EXPR:gm,IS_ALLOWED_URI:tc,IS_SCRIPT_OR_DATA:mm,MUSTACHE_EXPR:um,TMPLIT_EXPR:pm});const gn={element:1,text:3,progressingInstruction:7,comment:8,document:9},ym=function(){return typeof window>"u"?null:window},xm=function(t,n){if(typeof t!="object"||typeof t.createPolicy!="function")return null;let s=null;const i="data-tt-policy-suffix";n&&n.hasAttribute(i)&&(s=n.getAttribute(i));const a="dompurify"+(s?"#"+s:"");try{return t.createPolicy(a,{createHTML(o){return o},createScriptURL(o){return o}})}catch{return console.warn("TrustedTypes policy "+a+" could not be created."),null}},sr=function(){return{afterSanitizeAttributes:[],afterSanitizeElements:[],afterSanitizeShadowDOM:[],beforeSanitizeAttributes:[],beforeSanitizeElements:[],beforeSanitizeShadowDOM:[],uponSanitizeAttribute:[],uponSanitizeElement:[],uponSanitizeShadowNode:[]}};function sc(){let e=arguments.length>0&&arguments[0]!==void 0?arguments[0]:ym();const t=z=>sc(z);if(t.version="3.3.1",t.removed=[],!e||!e.document||e.document.nodeType!==gn.document||!e.Element)return t.isSupported=!1,t;let{document:n}=e;const s=n,i=s.currentScript,{DocumentFragment:a,HTMLTemplateElement:o,Node:r,Element:c,NodeFilter:d,NamedNodeMap:g=e.NamedNodeMap||e.MozNamedAttrMap,HTMLFormElement:p,DOMParser:h,trustedTypes:u}=e,f=c.prototype,v=un(f,"cloneNode"),w=un(f,"remove"),S=un(f,"nextSibling"),C=un(f,"childNodes"),k=un(f,"parentNode");if(typeof o=="function"){const z=n.createElement("template");z.content&&z.content.ownerDocument&&(n=z.content.ownerDocument)}let A,E="";const{implementation:T,createNodeIterator:I,createDocumentFragment:H,getElementsByTagName:Z}=n,{importNode:te}=s;let R=sr();t.isSupported=typeof ec=="function"&&typeof k=="function"&&T&&T.createHTMLDocument!==void 0;const{MUSTACHE_EXPR:q,ERB_EXPR:ne,TMPLIT_EXPR:oe,DATA_ATTR:_,ARIA_ATTR:j,IS_SCRIPT_OR_DATA:Q,ATTR_WHITESPACE:re,CUSTOM_ELEMENT:pe}=nr;let{IS_ALLOWED_URI:M}=nr,D=null;const N=W({},[...Xo,...ci,...di,...ui,...Jo]);let K=null;const ce=W({},[...er,...gi,...tr,...Qn]);let X=Object.seal(Mi(null,{tagNameCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},attributeNameCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},allowCustomizedBuiltInElements:{writable:!0,configurable:!1,enumerable:!0,value:!1}})),se=null,V=null;const U=Object.seal(Mi(null,{tagCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},attributeCheck:{writable:!0,configurable:!1,enumerable:!0,value:null}}));let ie=!0,le=!0,fe=!1,Te=!0,Ke=!1,st=!0,he=!1,Oe=!1,We=!1,qe=!1,Ve=!1,it=!1,at=!0,yt=!1;const Ns="user-content-";let jt=!0,ot=!1,Be={},Se=null;const sn=W({},["annotation-xml","audio","colgroup","desc","foreignobject","head","iframe","math","mi","mn","mo","ms","mtext","noembed","noframes","noscript","plaintext","script","style","svg","template","thead","title","video","xmp"]);let Kt=null;const rt=W({},["audio","video","img","source","image","track"]);let Fs=null;const Fa=W({},["alt","class","for","id","label","name","pattern","placeholder","role","summary","title","value","style","xmlns"]),Nn="http://www.w3.org/1998/Math/MathML",Fn="http://www.w3.org/2000/svg",Ge="http://www.w3.org/1999/xhtml";let Wt=Ge,Os=!1,Bs=null;const Tc=W({},[Nn,Fn,Ge],ri);let On=W({},["mi","mo","mn","ms","mtext"]),Bn=W({},["annotation-xml"]);const _c=W({},["title","style","font","a","script"]);let an=null;const Ec=["application/xhtml+xml","text/html"],Lc="text/html";let ue=null,qt=null;const Mc=n.createElement("form"),Oa=function(b){return b instanceof RegExp||b instanceof Function},zs=function(){let b=arguments.length>0&&arguments[0]!==void 0?arguments[0]:{};if(!(qt&&qt===b)){if((!b||typeof b!="object")&&(b={}),b=He(b),an=Ec.indexOf(b.PARSER_MEDIA_TYPE)===-1?Lc:b.PARSER_MEDIA_TYPE,ue=an==="application/xhtml+xml"?ri:os,D=Pe(b,"ALLOWED_TAGS")?W({},b.ALLOWED_TAGS,ue):N,K=Pe(b,"ALLOWED_ATTR")?W({},b.ALLOWED_ATTR,ue):ce,Bs=Pe(b,"ALLOWED_NAMESPACES")?W({},b.ALLOWED_NAMESPACES,ri):Tc,Fs=Pe(b,"ADD_URI_SAFE_ATTR")?W(He(Fa),b.ADD_URI_SAFE_ATTR,ue):Fa,Kt=Pe(b,"ADD_DATA_URI_TAGS")?W(He(rt),b.ADD_DATA_URI_TAGS,ue):rt,Se=Pe(b,"FORBID_CONTENTS")?W({},b.FORBID_CONTENTS,ue):sn,se=Pe(b,"FORBID_TAGS")?W({},b.FORBID_TAGS,ue):He({}),V=Pe(b,"FORBID_ATTR")?W({},b.FORBID_ATTR,ue):He({}),Be=Pe(b,"USE_PROFILES")?b.USE_PROFILES:!1,ie=b.ALLOW_ARIA_ATTR!==!1,le=b.ALLOW_DATA_ATTR!==!1,fe=b.ALLOW_UNKNOWN_PROTOCOLS||!1,Te=b.ALLOW_SELF_CLOSE_IN_ATTR!==!1,Ke=b.SAFE_FOR_TEMPLATES||!1,st=b.SAFE_FOR_XML!==!1,he=b.WHOLE_DOCUMENT||!1,qe=b.RETURN_DOM||!1,Ve=b.RETURN_DOM_FRAGMENT||!1,it=b.RETURN_TRUSTED_TYPE||!1,We=b.FORCE_BODY||!1,at=b.SANITIZE_DOM!==!1,yt=b.SANITIZE_NAMED_PROPS||!1,jt=b.KEEP_CONTENT!==!1,ot=b.IN_PLACE||!1,M=b.ALLOWED_URI_REGEXP||tc,Wt=b.NAMESPACE||Ge,On=b.MATHML_TEXT_INTEGRATION_POINTS||On,Bn=b.HTML_INTEGRATION_POINTS||Bn,X=b.CUSTOM_ELEMENT_HANDLING||{},b.CUSTOM_ELEMENT_HANDLING&&Oa(b.CUSTOM_ELEMENT_HANDLING.tagNameCheck)&&(X.tagNameCheck=b.CUSTOM_ELEMENT_HANDLING.tagNameCheck),b.CUSTOM_ELEMENT_HANDLING&&Oa(b.CUSTOM_ELEMENT_HANDLING.attributeNameCheck)&&(X.attributeNameCheck=b.CUSTOM_ELEMENT_HANDLING.attributeNameCheck),b.CUSTOM_ELEMENT_HANDLING&&typeof b.CUSTOM_ELEMENT_HANDLING.allowCustomizedBuiltInElements=="boolean"&&(X.allowCustomizedBuiltInElements=b.CUSTOM_ELEMENT_HANDLING.allowCustomizedBuiltInElements),Ke&&(le=!1),Ve&&(qe=!0),Be&&(D=W({},Jo),K=[],Be.html===!0&&(W(D,Xo),W(K,er)),Be.svg===!0&&(W(D,ci),W(K,gi),W(K,Qn)),Be.svgFilters===!0&&(W(D,di),W(K,gi),W(K,Qn)),Be.mathMl===!0&&(W(D,ui),W(K,tr),W(K,Qn))),b.ADD_TAGS&&(typeof b.ADD_TAGS=="function"?U.tagCheck=b.ADD_TAGS:(D===N&&(D=He(D)),W(D,b.ADD_TAGS,ue))),b.ADD_ATTR&&(typeof b.ADD_ATTR=="function"?U.attributeCheck=b.ADD_ATTR:(K===ce&&(K=He(K)),W(K,b.ADD_ATTR,ue))),b.ADD_URI_SAFE_ATTR&&W(Fs,b.ADD_URI_SAFE_ATTR,ue),b.FORBID_CONTENTS&&(Se===sn&&(Se=He(Se)),W(Se,b.FORBID_CONTENTS,ue)),b.ADD_FORBID_CONTENTS&&(Se===sn&&(Se=He(Se)),W(Se,b.ADD_FORBID_CONTENTS,ue)),jt&&(D["#text"]=!0),he&&W(D,["html","head","body"]),D.table&&(W(D,["tbody"]),delete se.tbody),b.TRUSTED_TYPES_POLICY){if(typeof b.TRUSTED_TYPES_POLICY.createHTML!="function")throw dn('TRUSTED_TYPES_POLICY configuration option must provide a "createHTML" hook.');if(typeof b.TRUSTED_TYPES_POLICY.createScriptURL!="function")throw dn('TRUSTED_TYPES_POLICY configuration option must provide a "createScriptURL" hook.');A=b.TRUSTED_TYPES_POLICY,E=A.createHTML("")}else A===void 0&&(A=xm(u,i)),A!==null&&typeof E=="string"&&(E=A.createHTML(""));we&&we(b),qt=b}},Ba=W({},[...ci,...di,...cm]),za=W({},[...ui,...dm]),Ic=function(b){let L=k(b);(!L||!L.tagName)&&(L={namespaceURI:Wt,tagName:"template"});const O=os(b.tagName),ae=os(L.tagName);return Bs[b.namespaceURI]?b.namespaceURI===Fn?L.namespaceURI===Ge?O==="svg":L.namespaceURI===Nn?O==="svg"&&(ae==="annotation-xml"||On[ae]):!!Ba[O]:b.namespaceURI===Nn?L.namespaceURI===Ge?O==="math":L.namespaceURI===Fn?O==="math"&&Bn[ae]:!!za[O]:b.namespaceURI===Ge?L.namespaceURI===Fn&&!Bn[ae]||L.namespaceURI===Nn&&!On[ae]?!1:!za[O]&&(_c[O]||!Ba[O]):!!(an==="application/xhtml+xml"&&Bs[b.namespaceURI]):!1},ze=function(b){ln(t.removed,{element:b});try{k(b).removeChild(b)}catch{w(b)}},xt=function(b,L){try{ln(t.removed,{attribute:L.getAttributeNode(b),from:L})}catch{ln(t.removed,{attribute:null,from:L})}if(L.removeAttribute(b),b==="is")if(qe||Ve)try{ze(L)}catch{}else try{L.setAttribute(b,"")}catch{}},Ua=function(b){let L=null,O=null;if(We)b="<remove></remove>"+b;else{const de=li(b,/^[\r\n\t ]+/);O=de&&de[0]}an==="application/xhtml+xml"&&Wt===Ge&&(b='<html xmlns="http://www.w3.org/1999/xhtml"><head></head><body>'+b+"</body></html>");const ae=A?A.createHTML(b):b;if(Wt===Ge)try{L=new h().parseFromString(ae,an)}catch{}if(!L||!L.documentElement){L=T.createDocument(Wt,"template",null);try{L.documentElement.innerHTML=Os?E:ae}catch{}}const be=L.body||L.documentElement;return b&&O&&be.insertBefore(n.createTextNode(O),be.childNodes[0]||null),Wt===Ge?Z.call(L,he?"html":"body")[0]:he?L.documentElement:be},Ha=function(b){return I.call(b.ownerDocument||b,b,d.SHOW_ELEMENT|d.SHOW_COMMENT|d.SHOW_TEXT|d.SHOW_PROCESSING_INSTRUCTION|d.SHOW_CDATA_SECTION,null)},Us=function(b){return b instanceof p&&(typeof b.nodeName!="string"||typeof b.textContent!="string"||typeof b.removeChild!="function"||!(b.attributes instanceof g)||typeof b.removeAttribute!="function"||typeof b.setAttribute!="function"||typeof b.namespaceURI!="string"||typeof b.insertBefore!="function"||typeof b.hasChildNodes!="function")},ja=function(b){return typeof r=="function"&&b instanceof r};function Qe(z,b,L){Gn(z,O=>{O.call(t,b,L,qt)})}const Ka=function(b){let L=null;if(Qe(R.beforeSanitizeElements,b,null),Us(b))return ze(b),!0;const O=ue(b.nodeName);if(Qe(R.uponSanitizeElement,b,{tagName:O,allowedTags:D}),st&&b.hasChildNodes()&&!ja(b.firstElementChild)&&xe(/<[/\w!]/g,b.innerHTML)&&xe(/<[/\w!]/g,b.textContent)||b.nodeType===gn.progressingInstruction||st&&b.nodeType===gn.comment&&xe(/<[/\w]/g,b.data))return ze(b),!0;if(!(U.tagCheck instanceof Function&&U.tagCheck(O))&&(!D[O]||se[O])){if(!se[O]&&qa(O)&&(X.tagNameCheck instanceof RegExp&&xe(X.tagNameCheck,O)||X.tagNameCheck instanceof Function&&X.tagNameCheck(O)))return!1;if(jt&&!Se[O]){const ae=k(b)||b.parentNode,be=C(b)||b.childNodes;if(be&&ae){const de=be.length;for(let Ae=de-1;Ae>=0;--Ae){const Ye=v(be[Ae],!0);Ye.__removalCount=(b.__removalCount||0)+1,ae.insertBefore(Ye,S(b))}}}return ze(b),!0}return b instanceof c&&!Ic(b)||(O==="noscript"||O==="noembed"||O==="noframes")&&xe(/<\/no(script|embed|frames)/i,b.innerHTML)?(ze(b),!0):(Ke&&b.nodeType===gn.text&&(L=b.textContent,Gn([q,ne,oe],ae=>{L=cn(L,ae," ")}),b.textContent!==L&&(ln(t.removed,{element:b.cloneNode()}),b.textContent=L)),Qe(R.afterSanitizeElements,b,null),!1)},Wa=function(b,L,O){if(at&&(L==="id"||L==="name")&&(O in n||O in Mc))return!1;if(!(le&&!V[L]&&xe(_,L))){if(!(ie&&xe(j,L))){if(!(U.attributeCheck instanceof Function&&U.attributeCheck(L,b))){if(!K[L]||V[L]){if(!(qa(b)&&(X.tagNameCheck instanceof RegExp&&xe(X.tagNameCheck,b)||X.tagNameCheck instanceof Function&&X.tagNameCheck(b))&&(X.attributeNameCheck instanceof RegExp&&xe(X.attributeNameCheck,L)||X.attributeNameCheck instanceof Function&&X.attributeNameCheck(L,b))||L==="is"&&X.allowCustomizedBuiltInElements&&(X.tagNameCheck instanceof RegExp&&xe(X.tagNameCheck,O)||X.tagNameCheck instanceof Function&&X.tagNameCheck(O))))return!1}else if(!Fs[L]){if(!xe(M,cn(O,re,""))){if(!((L==="src"||L==="xlink:href"||L==="href")&&b!=="script"&&am(O,"data:")===0&&Kt[b])){if(!(fe&&!xe(Q,cn(O,re,"")))){if(O)return!1}}}}}}}return!0},qa=function(b){return b!=="annotation-xml"&&li(b,pe)},Va=function(b){Qe(R.beforeSanitizeAttributes,b,null);const{attributes:L}=b;if(!L||Us(b))return;const O={attrName:"",attrValue:"",keepAttr:!0,allowedAttributes:K,forceKeepAttr:void 0};let ae=L.length;for(;ae--;){const be=L[ae],{name:de,namespaceURI:Ae,value:Ye}=be,Vt=ue(de),Hs=Ye;let me=de==="value"?Hs:om(Hs);if(O.attrName=Vt,O.attrValue=me,O.keepAttr=!0,O.forceKeepAttr=void 0,Qe(R.uponSanitizeAttribute,b,O),me=O.attrValue,yt&&(Vt==="id"||Vt==="name")&&(xt(de,b),me=Ns+me),st&&xe(/((--!?|])>)|<\/(style|title|textarea)/i,me)){xt(de,b);continue}if(Vt==="attributename"&&li(me,"href")){xt(de,b);continue}if(O.forceKeepAttr)continue;if(!O.keepAttr){xt(de,b);continue}if(!Te&&xe(/\/>/i,me)){xt(de,b);continue}Ke&&Gn([q,ne,oe],Qa=>{me=cn(me,Qa," ")});const Ga=ue(b.nodeName);if(!Wa(Ga,Vt,me)){xt(de,b);continue}if(A&&typeof u=="object"&&typeof u.getAttributeType=="function"&&!Ae)switch(u.getAttributeType(Ga,Vt)){case"TrustedHTML":{me=A.createHTML(me);break}case"TrustedScriptURL":{me=A.createScriptURL(me);break}}if(me!==Hs)try{Ae?b.setAttributeNS(Ae,de,me):b.setAttribute(de,me),Us(b)?ze(b):Zo(t.removed)}catch{xt(de,b)}}Qe(R.afterSanitizeAttributes,b,null)},Rc=function z(b){let L=null;const O=Ha(b);for(Qe(R.beforeSanitizeShadowDOM,b,null);L=O.nextNode();)Qe(R.uponSanitizeShadowNode,L,null),Ka(L),Va(L),L.content instanceof a&&z(L.content);Qe(R.afterSanitizeShadowDOM,b,null)};return t.sanitize=function(z){let b=arguments.length>1&&arguments[1]!==void 0?arguments[1]:{},L=null,O=null,ae=null,be=null;if(Os=!z,Os&&(z="<!-->"),typeof z!="string"&&!ja(z))if(typeof z.toString=="function"){if(z=z.toString(),typeof z!="string")throw dn("dirty is not a string, aborting")}else throw dn("toString is not a function");if(!t.isSupported)return z;if(Oe||zs(b),t.removed=[],typeof z=="string"&&(ot=!1),ot){if(z.nodeName){const Ye=ue(z.nodeName);if(!D[Ye]||se[Ye])throw dn("root node is forbidden and cannot be sanitized in-place")}}else if(z instanceof r)L=Ua("<!---->"),O=L.ownerDocument.importNode(z,!0),O.nodeType===gn.element&&O.nodeName==="BODY"||O.nodeName==="HTML"?L=O:L.appendChild(O);else{if(!qe&&!Ke&&!he&&z.indexOf("<")===-1)return A&&it?A.createHTML(z):z;if(L=Ua(z),!L)return qe?null:it?E:""}L&&We&&ze(L.firstChild);const de=Ha(ot?z:L);for(;ae=de.nextNode();)Ka(ae),Va(ae),ae.content instanceof a&&Rc(ae.content);if(ot)return z;if(qe){if(Ve)for(be=H.call(L.ownerDocument);L.firstChild;)be.appendChild(L.firstChild);else be=L;return(K.shadowroot||K.shadowrootmode)&&(be=te.call(s,be,!0)),be}let Ae=he?L.outerHTML:L.innerHTML;return he&&D["!doctype"]&&L.ownerDocument&&L.ownerDocument.doctype&&L.ownerDocument.doctype.name&&xe(nc,L.ownerDocument.doctype.name)&&(Ae="<!DOCTYPE "+L.ownerDocument.doctype.name+`>
`+Ae),Ke&&Gn([q,ne,oe],Ye=>{Ae=cn(Ae,Ye," ")}),A&&it?A.createHTML(Ae):Ae},t.setConfig=function(){let z=arguments.length>0&&arguments[0]!==void 0?arguments[0]:{};zs(z),Oe=!0},t.clearConfig=function(){qt=null,Oe=!1},t.isValidAttribute=function(z,b,L){qt||zs({});const O=ue(z),ae=ue(b);return Wa(O,ae,L)},t.addHook=function(z,b){typeof b=="function"&&ln(R[z],b)},t.removeHook=function(z,b){if(b!==void 0){const L=sm(R[z],b);return L===-1?void 0:im(R[z],L,1)[0]}return Zo(R[z])},t.removeHooks=function(z){R[z]=[]},t.removeAllHooks=function(){R=sr()},t}var Pi=sc();function wa(){return{async:!1,breaks:!1,extensions:null,gfm:!0,hooks:null,pedantic:!1,renderer:null,silent:!1,tokenizer:null,walkTokens:null}}var Ht=wa();function ic(e){Ht=e}var Mt={exec:()=>null};function G(e,t=""){let n=typeof e=="string"?e:e.source,s={replace:(i,a)=>{let o=typeof a=="string"?a:a.source;return o=o.replace($e.caret,"$1"),n=n.replace(i,o),s},getRegex:()=>new RegExp(n,t)};return s}var $m=(()=>{try{return!!new RegExp("(?<=1)(?<!1)")}catch{return!1}})(),$e={codeRemoveIndent:/^(?: {1,4}| {0,3}\t)/gm,outputLinkReplace:/\\([\[\]])/g,indentCodeCompensation:/^(\s+)(?:```)/,beginningSpace:/^\s+/,endingHash:/#$/,startingSpaceChar:/^ /,endingSpaceChar:/ $/,nonSpaceChar:/[^ ]/,newLineCharGlobal:/\n/g,tabCharGlobal:/\t/g,multipleSpaceGlobal:/\s+/g,blankLine:/^[ \t]*$/,doubleBlankLine:/\n[ \t]*\n[ \t]*$/,blockquoteStart:/^ {0,3}>/,blockquoteSetextReplace:/\n {0,3}((?:=+|-+) *)(?=\n|$)/g,blockquoteSetextReplace2:/^ {0,3}>[ \t]?/gm,listReplaceNesting:/^ {1,4}(?=( {4})*[^ ])/g,listIsTask:/^\[[ xX]\] +\S/,listReplaceTask:/^\[[ xX]\] +/,listTaskCheckbox:/\[[ xX]\]/,anyLine:/\n.*\n/,hrefBrackets:/^<(.*)>$/,tableDelimiter:/[:|]/,tableAlignChars:/^\||\| *$/g,tableRowBlankLine:/\n[ \t]*$/,tableAlignRight:/^ *-+: *$/,tableAlignCenter:/^ *:-+: *$/,tableAlignLeft:/^ *:-+ *$/,startATag:/^<a /i,endATag:/^<\/a>/i,startPreScriptTag:/^<(pre|code|kbd|script)(\s|>)/i,endPreScriptTag:/^<\/(pre|code|kbd|script)(\s|>)/i,startAngleBracket:/^</,endAngleBracket:/>$/,pedanticHrefTitle:/^([^'"]*[^\s])\s+(['"])(.*)\2/,unicodeAlphaNumeric:/[\p{L}\p{N}]/u,escapeTest:/[&<>"']/,escapeReplace:/[&<>"']/g,escapeTestNoEncode:/[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/,escapeReplaceNoEncode:/[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/g,unescapeTest:/&(#(?:\d+)|(?:#x[0-9A-Fa-f]+)|(?:\w+));?/ig,caret:/(^|[^\[])\^/g,percentDecode:/%25/g,findPipe:/\|/g,splitPipe:/ \|/,slashPipe:/\\\|/g,carriageReturn:/\r\n|\r/g,spaceLine:/^ +$/gm,notSpaceStart:/^\S*/,endingNewline:/\n$/,listItemRegex:e=>new RegExp(`^( {0,3}${e})((?:[	 ][^\\n]*)?(?:\\n|$))`),nextBulletRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`),hrRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`),fencesBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}(?:\`\`\`|~~~)`),headingBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}#`),htmlBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}<(?:[a-z].*>|!--)`,"i"),blockquoteBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}>`)},wm=/^(?:[ \t]*(?:\n|$))+/,km=/^((?: {4}| {0,3}\t)[^\n]+(?:\n(?:[ \t]*(?:\n|$))*)?)+/,Sm=/^ {0,3}(`{3,}(?=[^`\n]*(?:\n|$))|~{3,})([^\n]*)(?:\n|$)(?:|([\s\S]*?)(?:\n|$))(?: {0,3}\1[~`]* *(?=\n|$)|$)/,Dn=/^ {0,3}((?:-[\t ]*){3,}|(?:_[ \t]*){3,}|(?:\*[ \t]*){3,})(?:\n+|$)/,Am=/^ {0,3}(#{1,6})(?=\s|$)(.*)(?:\n+|$)/,ka=/ {0,3}(?:[*+-]|\d{1,9}[.)])/,ac=/^(?!bull |blockCode|fences|blockquote|heading|html|table)((?:.|\n(?!\s*?\n|bull |blockCode|fences|blockquote|heading|html|table))+?)\n {0,3}(=+|-+) *(?:\n+|$)/,oc=G(ac).replace(/bull/g,ka).replace(/blockCode/g,/(?: {4}| {0,3}\t)/).replace(/fences/g,/ {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g,/ {0,3}>/).replace(/heading/g,/ {0,3}#{1,6}/).replace(/html/g,/ {0,3}<[^\n>]+>\n/).replace(/\|table/g,"").getRegex(),Cm=G(ac).replace(/bull/g,ka).replace(/blockCode/g,/(?: {4}| {0,3}\t)/).replace(/fences/g,/ {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g,/ {0,3}>/).replace(/heading/g,/ {0,3}#{1,6}/).replace(/html/g,/ {0,3}<[^\n>]+>\n/).replace(/table/g,/ {0,3}\|?(?:[:\- ]*\|)+[\:\- ]*\n/).getRegex(),Sa=/^([^\n]+(?:\n(?!hr|heading|lheading|blockquote|fences|list|html|table| +\n)[^\n]+)*)/,Tm=/^[^\n]+/,Aa=/(?!\s*\])(?:\\[\s\S]|[^\[\]\\])+/,_m=G(/^ {0,3}\[(label)\]: *(?:\n[ \t]*)?([^<\s][^\s]*|<.*?>)(?:(?: +(?:\n[ \t]*)?| *\n[ \t]*)(title))? *(?:\n+|$)/).replace("label",Aa).replace("title",/(?:"(?:\\"?|[^"\\])*"|'[^'\n]*(?:\n[^'\n]+)*\n?'|\([^()]*\))/).getRegex(),Em=G(/^(bull)([ \t][^\n]+?)?(?:\n|$)/).replace(/bull/g,ka).getRegex(),Rs="address|article|aside|base|basefont|blockquote|body|caption|center|col|colgroup|dd|details|dialog|dir|div|dl|dt|fieldset|figcaption|figure|footer|form|frame|frameset|h[1-6]|head|header|hr|html|iframe|legend|li|link|main|menu|menuitem|meta|nav|noframes|ol|optgroup|option|p|param|search|section|summary|table|tbody|td|tfoot|th|thead|title|tr|track|ul",Ca=/<!--(?:-?>|[\s\S]*?(?:-->|$))/,Lm=G("^ {0,3}(?:<(script|pre|style|textarea)[\\s>][\\s\\S]*?(?:</\\1>[^\\n]*\\n+|$)|comment[^\\n]*(\\n+|$)|<\\?[\\s\\S]*?(?:\\?>\\n*|$)|<![A-Z][\\s\\S]*?(?:>\\n*|$)|<!\\[CDATA\\[[\\s\\S]*?(?:\\]\\]>\\n*|$)|</?(tag)(?: +|\\n|/?>)[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|<(?!script|pre|style|textarea)([a-z][\\w-]*)(?:attribute)*? */?>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|</(?!script|pre|style|textarea)[a-z][\\w-]*\\s*>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$))","i").replace("comment",Ca).replace("tag",Rs).replace("attribute",/ +[a-zA-Z:_][\w.:-]*(?: *= *"[^"\n]*"| *= *'[^'\n]*'| *= *[^\s"'=<>`]+)?/).getRegex(),rc=G(Sa).replace("hr",Dn).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("|lheading","").replace("|table","").replace("blockquote"," {0,3}>").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",Rs).getRegex(),Mm=G(/^( {0,3}> ?(paragraph|[^\n]*)(?:\n|$))+/).replace("paragraph",rc).getRegex(),Ta={blockquote:Mm,code:km,def:_m,fences:Sm,heading:Am,hr:Dn,html:Lm,lheading:oc,list:Em,newline:wm,paragraph:rc,table:Mt,text:Tm},ir=G("^ *([^\\n ].*)\\n {0,3}((?:\\| *)?:?-+:? *(?:\\| *:?-+:? *)*(?:\\| *)?)(?:\\n((?:(?! *\\n|hr|heading|blockquote|code|fences|list|html).*(?:\\n|$))*)\\n*|$)").replace("hr",Dn).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("blockquote"," {0,3}>").replace("code","(?: {4}| {0,3}	)[^\\n]").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",Rs).getRegex(),Im={...Ta,lheading:Cm,table:ir,paragraph:G(Sa).replace("hr",Dn).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("|lheading","").replace("table",ir).replace("blockquote"," {0,3}>").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",Rs).getRegex()},Rm={...Ta,html:G(`^ *(?:comment *(?:\\n|\\s*$)|<(tag)[\\s\\S]+?</\\1> *(?:\\n{2,}|\\s*$)|<tag(?:"[^"]*"|'[^']*'|\\s[^'"/>\\s]*)*?/?> *(?:\\n{2,}|\\s*$))`).replace("comment",Ca).replace(/tag/g,"(?!(?:a|em|strong|small|s|cite|q|dfn|abbr|data|time|code|var|samp|kbd|sub|sup|i|b|u|mark|ruby|rt|rp|bdi|bdo|span|br|wbr|ins|del|img)\\b)\\w+(?!:|[^\\w\\s@]*@)\\b").getRegex(),def:/^ *\[([^\]]+)\]: *<?([^\s>]+)>?(?: +(["(][^\n]+[")]))? *(?:\n+|$)/,heading:/^(#{1,6})(.*)(?:\n+|$)/,fences:Mt,lheading:/^(.+?)\n {0,3}(=+|-+) *(?:\n+|$)/,paragraph:G(Sa).replace("hr",Dn).replace("heading",` *#{1,6} *[^
]`).replace("lheading",oc).replace("|table","").replace("blockquote"," {0,3}>").replace("|fences","").replace("|list","").replace("|html","").replace("|tag","").getRegex()},Pm=/^\\([!"#$%&'()*+,\-./:;<=>?@\[\]\\^_`{|}~])/,Dm=/^(`+)([^`]|[^`][\s\S]*?[^`])\1(?!`)/,lc=/^( {2,}|\\)\n(?!\s*$)/,Nm=/^(`+|[^`])(?:(?= {2,}\n)|[\s\S]*?(?:(?=[\\<!\[`*_]|\b_|$)|[^ ](?= {2,}\n)))/,Ps=/[\p{P}\p{S}]/u,_a=/[\s\p{P}\p{S}]/u,cc=/[^\s\p{P}\p{S}]/u,Fm=G(/^((?![*_])punctSpace)/,"u").replace(/punctSpace/g,_a).getRegex(),dc=/(?!~)[\p{P}\p{S}]/u,Om=/(?!~)[\s\p{P}\p{S}]/u,Bm=/(?:[^\s\p{P}\p{S}]|~)/u,uc=/(?![*_])[\p{P}\p{S}]/u,zm=/(?![*_])[\s\p{P}\p{S}]/u,Um=/(?:[^\s\p{P}\p{S}]|[*_])/u,Hm=G(/link|precode-code|html/,"g").replace("link",/\[(?:[^\[\]`]|(?<a>`+)[^`]+\k<a>(?!`))*?\]\((?:\\[\s\S]|[^\\\(\)]|\((?:\\[\s\S]|[^\\\(\)])*\))*\)/).replace("precode-",$m?"(?<!`)()":"(^^|[^`])").replace("code",/(?<b>`+)[^`]+\k<b>(?!`)/).replace("html",/<(?! )[^<>]*?>/).getRegex(),gc=/^(?:\*+(?:((?!\*)punct)|[^\s*]))|^_+(?:((?!_)punct)|([^\s_]))/,jm=G(gc,"u").replace(/punct/g,Ps).getRegex(),Km=G(gc,"u").replace(/punct/g,dc).getRegex(),pc="^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)",Wm=G(pc,"gu").replace(/notPunctSpace/g,cc).replace(/punctSpace/g,_a).replace(/punct/g,Ps).getRegex(),qm=G(pc,"gu").replace(/notPunctSpace/g,Bm).replace(/punctSpace/g,Om).replace(/punct/g,dc).getRegex(),Vm=G("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)punct(_+)(?=[\\s]|$)|notPunctSpace(_+)(?!_)(?=punctSpace|$)|(?!_)punctSpace(_+)(?=notPunctSpace)|[\\s](_+)(?!_)(?=punct)|(?!_)punct(_+)(?!_)(?=punct)","gu").replace(/notPunctSpace/g,cc).replace(/punctSpace/g,_a).replace(/punct/g,Ps).getRegex(),Gm=G(/^~~?(?:((?!~)punct)|[^\s~])/,"u").replace(/punct/g,uc).getRegex(),Qm="^[^~]+(?=[^~])|(?!~)punct(~~?)(?=[\\s]|$)|notPunctSpace(~~?)(?!~)(?=punctSpace|$)|(?!~)punctSpace(~~?)(?=notPunctSpace)|[\\s](~~?)(?!~)(?=punct)|(?!~)punct(~~?)(?!~)(?=punct)|notPunctSpace(~~?)(?=notPunctSpace)",Ym=G(Qm,"gu").replace(/notPunctSpace/g,Um).replace(/punctSpace/g,zm).replace(/punct/g,uc).getRegex(),Zm=G(/\\(punct)/,"gu").replace(/punct/g,Ps).getRegex(),Xm=G(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/).replace("scheme",/[a-zA-Z][a-zA-Z0-9+.-]{1,31}/).replace("email",/[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/).getRegex(),Jm=G(Ca).replace("(?:-->|$)","-->").getRegex(),ev=G("^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>").replace("comment",Jm).replace("attribute",/\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/).getRegex(),ms=/(?:\[(?:\\[\s\S]|[^\[\]\\])*\]|\\[\s\S]|`+[^`]*?`+(?!`)|[^\[\]\\`])*?/,tv=G(/^!?\[(label)\]\(\s*(href)(?:(?:[ \t]*(?:\n[ \t]*)?)(title))?\s*\)/).replace("label",ms).replace("href",/<(?:\\.|[^\n<>\\])+>|[^ \t\n\x00-\x1f]*/).replace("title",/"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/).getRegex(),fc=G(/^!?\[(label)\]\[(ref)\]/).replace("label",ms).replace("ref",Aa).getRegex(),hc=G(/^!?\[(ref)\](?:\[\])?/).replace("ref",Aa).getRegex(),nv=G("reflink|nolink(?!\\()","g").replace("reflink",fc).replace("nolink",hc).getRegex(),ar=/[hH][tT][tT][pP][sS]?|[fF][tT][pP]/,Ea={_backpedal:Mt,anyPunctuation:Zm,autolink:Xm,blockSkip:Hm,br:lc,code:Dm,del:Mt,delLDelim:Mt,delRDelim:Mt,emStrongLDelim:jm,emStrongRDelimAst:Wm,emStrongRDelimUnd:Vm,escape:Pm,link:tv,nolink:hc,punctuation:Fm,reflink:fc,reflinkSearch:nv,tag:ev,text:Nm,url:Mt},sv={...Ea,link:G(/^!?\[(label)\]\((.*?)\)/).replace("label",ms).getRegex(),reflink:G(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label",ms).getRegex()},Di={...Ea,emStrongRDelimAst:qm,emStrongLDelim:Km,delLDelim:Gm,delRDelim:Ym,url:G(/^((?:protocol):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/).replace("protocol",ar).replace("email",/[A-Za-z0-9._+-]+(@)[a-zA-Z0-9-_]+(?:\.[a-zA-Z0-9-_]*[a-zA-Z0-9])+(?![-_])/).getRegex(),_backpedal:/(?:[^?!.,:;*_'"~()&]+|\([^)]*\)|&(?![a-zA-Z0-9]+;$)|[?!.,:;*_'"~)]+(?!$))+/,del:/^(~~?)(?=[^\s~])((?:\\[\s\S]|[^\\])*?(?:\\[\s\S]|[^\s~\\]))\1(?=[^~]|$)/,text:G(/^([`~]+|[^`~])(?:(?= {2,}\n)|(?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)|[\s\S]*?(?:(?=[\\<!\[`*~_]|\b_|protocol:\/\/|www\.|$)|[^ ](?= {2,}\n)|[^a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-](?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)))/).replace("protocol",ar).getRegex()},iv={...Di,br:G(lc).replace("{2,}","*").getRegex(),text:G(Di.text).replace("\\b_","\\b_| {2,}\\n").replace(/\{2,\}/g,"*").getRegex()},Yn={normal:Ta,gfm:Im,pedantic:Rm},pn={normal:Ea,gfm:Di,breaks:iv,pedantic:sv},av={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"},or=e=>av[e];function et(e,t){if(t){if($e.escapeTest.test(e))return e.replace($e.escapeReplace,or)}else if($e.escapeTestNoEncode.test(e))return e.replace($e.escapeReplaceNoEncode,or);return e}function rr(e){try{e=encodeURI(e).replace($e.percentDecode,"%")}catch{return null}return e}function lr(e,t){let n=e.replace($e.findPipe,(a,o,r)=>{let c=!1,d=o;for(;--d>=0&&r[d]==="\\";)c=!c;return c?"|":" |"}),s=n.split($e.splitPipe),i=0;if(s[0].trim()||s.shift(),s.length>0&&!s.at(-1)?.trim()&&s.pop(),t)if(s.length>t)s.splice(t);else for(;s.length<t;)s.push("");for(;i<s.length;i++)s[i]=s[i].trim().replace($e.slashPipe,"|");return s}function fn(e,t,n){let s=e.length;if(s===0)return"";let i=0;for(;i<s&&e.charAt(s-i-1)===t;)i++;return e.slice(0,s-i)}function ov(e,t){if(e.indexOf(t[1])===-1)return-1;let n=0;for(let s=0;s<e.length;s++)if(e[s]==="\\")s++;else if(e[s]===t[0])n++;else if(e[s]===t[1]&&(n--,n<0))return s;return n>0?-2:-1}function rv(e,t=0){let n=t,s="";for(let i of e)if(i==="	"){let a=4-n%4;s+=" ".repeat(a),n+=a}else s+=i,n++;return s}function cr(e,t,n,s,i){let a=t.href,o=t.title||null,r=e[1].replace(i.other.outputLinkReplace,"$1");s.state.inLink=!0;let c={type:e[0].charAt(0)==="!"?"image":"link",raw:n,href:a,title:o,text:r,tokens:s.inlineTokens(r)};return s.state.inLink=!1,c}function lv(e,t,n){let s=e.match(n.other.indentCodeCompensation);if(s===null)return t;let i=s[1];return t.split(`
...
`:""}*No output — tool completed successfully.*`;t(p)}:void 0,r=i&&(e.text?.length??0)<=Vv,c=i&&!r,d=i&&r,g=!i;return l`
  ...
  `}const Da=["#e879f9","#38bdf8","#34d399","#fb923c","#a78bfa","#f472b6","#2dd4bf","#fbbf24","#818cf8","#4ade80"];function ib(e){let t=0;for(let n=0;n<e.length;n++)t=(t<<5)-t+e.charCodeAt(n)|0;return Math.abs(t)%Da.length}function ab(e){if(e.role!=="assistant"||!e.messages.length)return null;const t=e.messages[0]?.message;if(!t?.content)return null;const n=t.content;if(!Array.isArray(n))return null;for(const s of n)if(s.type==="text"&&typeof s.text=="string"){const i=s.text.match(/^\[([A-Za-z0-9_-]+(?:\s*→\s*[A-Za-z0-9_-]+)?)\]\s/);if(i)return i[1]}return null}function ob(e,t){const n=Da[t],s=e.split(/\s*→\s*/),a=(s[s.length-1]||e).charAt(0).toUpperCase();return l`<div class="chat-avatar injected" style="background: ${n}; color: #fff;">${a}</div>`}function rb(e,t){const n=Ia(e.role),s=t.assistantName??"Assistant",i=ab(e),a=i!==null,o=a?ib(i):-1,r=a?i:n==="user"?"You":n==="assistant"?s:n,c=a?"injected":n==="user"?"user":n==="assistant"?"assistant":"other",d=new Date(e.timestamp).toLocaleTimeString([],{hour:"numeric",minute:"2-digit"}),g=a?`--injected-color: ${Da[o]}`:"";return l`
  ...
  `}function Na(e,t){const n=Ia(e),s=t?.name?.trim()||"Assistant",i=t?.avatar?.trim()||"",a=n==="user"?"U":n==="assistant"?s.charAt(0).toUpperCase()||"A":n==="tool"?"⚙":"?",o=n==="user"?"user":n==="assistant"?"assistant":n==="tool"?"tool":"other";return i&&n==="assistant"?lb(i)?l`<img
        class="chat-avatar ${o}"
        src="${i}"
        alt="${s}"
  ...
  `}var ub=Object.defineProperty,gb=Object.getOwnPropertyDescriptor,Ds=(e,t,n,s)=>{for(var i=s>1?void 0:s?gb(t,n):t,a=e.length-1,o;a>=0;a--)(o=e[a])&&(i=(s?o(t,n,i):o(i))||i);return s&&i&&ub(t,n,i),i};let en=class extends Zt{constructor(){super(...arguments),this.splitRatio=.6,this.minRatio=.4,this.maxRatio=.7,this.isDragging=!1,this.startX=0,this.startRatio=0,this.handleMouseDown=e=>{this.isDragging=!0,this.startX=e.clientX,this.startRatio=this.splitRatio,this.classList.add("dragging"),document.addEventListener("mousemove",this.handleMouseMove),document.addEventListener("mouseup",this.handleMouseUp),e.preventDefault()},this.handleMouseMove=e=>{if(!this.isDragging)return;const t=this.parentElement;if(!t)return;const n=t.getBoundingClientRect().width,i=(e.clientX-this.startX)/n;let a=this.startRatio+i;a=Math.max(this.minRatio,Math.min(this.maxRatio,a)),this.dispatchEvent(new CustomEvent("resize",{detail:{splitRatio:a},bubbles:!0,composed:!0}))},this.handleMouseUp=()=>{this.isDragging=!1,this.classList.remove("dragging"),document.removeEventListener("mousemove",this.handleMouseMove),document.removeEventListener("mouseup",this.handleMouseUp)}}render(){return m}connectedCallback(){super.connectedCallback(),this.addEventListener("mousedown",this.handleMouseDown)}disconnectedCallback(){super.disconnectedCallback(),this.removeEventListener("mousedown",this.handleMouseDown),document.removeEventListener("mousemove",this.handleMouseMove),document.removeEventListener("mouseup",this.handleMouseUp)}};en.styles=Dc`
    :host {
      width: 4px;
      cursor: col-resize;
  ...
  `;Ds([ws({type:Number})],en.prototype,"splitRatio",2);Ds([ws({type:Number})],en.prototype,"minRatio",2);Ds([ws({type:Number})],en.prototype,"maxRatio",2);en=Ds([Nr("resizable-divider")],en);const pb=5e3;function vr(e){e.style.height="auto",e.style.height=`${e.scrollHeight}px`}function fb(e){return e?e.active?l`
...
\`\`\``)}})}
        ...
        <div class="chat-compose__row">
          <label class="field chat-compose__field">
            ...
            <textarea
              ...
              @input=${f=>{const v=f.target;vr(v),e.onDraftChange(v.value)}}
              @paste=${f=>mb(f,e)}
              placeholder=${d}
            ></textarea>
          </label>
          <div class="chat-compose__actions">
            ${s?l`<button
                ...
                >Stop</button>`:l`${(e.agentsList?.agents??[]).length>0?l`<select
                        ...
                        @change=${f=>{const v=f.target,w=v.value;v.value="",w==="__new__"?e.onNewSession():w&&e.onSelectAgent&&e.onSelectAgent(w)}}
                      >
                        <option value="" disabled selected>+ New ▾</option>
                        <option value="__new__">New chat (current agent)</option>
                        ${(e.agentsList?.agents??[]).map(f=>{const v=f.identity?.name||f.name||f.id;return l`<option value=${f.id}>New chat with ${v}</option>`})}
                      </select>`:l`<button
                        class="btn"
                        ?disabled=${!e.connected||e.sending}
  ...
  `},xr=[{key:"env",label:"Environment"},{key:"update",label:"Updates"},{key:"agents",label:"Agents"},{key:"auth",label:"Authentication"},{key:"channels",label:"Channels"},{key:"messages",label:"Messages"},{key:"commands",label:"Commands"},{key:"hooks",label:"Hooks"},{key:"skills",label:"Skills"},{key:"tools",label:"Tools"},{key:"gateway",label:"Gateway"},{key:"wizard",label:"Setup Wizard"}],$r="__all__";function wr(e){return Bi[e]??Bi.default}function $b(e,t){const n=$a[e];return n||{label:t?.title??tt(e),description:t?.description??""}}function wb(e){const{key:t,schema:n,uiHints:s}=e;if(!n||Ee(n)!=="object"||!n.properties)return[];const i=Object.entries(n.properties).map(([a,o])=>{const r=Ie([t,a],s),c=r?.label??o.title??tt(a),d=r?.help??o.description??"",g=r?.order??50;return{key:a,label:c,description:d,order:g}});return i.sort((a,o)=>a.order!==o.order?a.order-o.order:a.key.localeCompare(o.key)),i}function kb(e,t){if(!e||!t)return[];const n=[];function s(i,a,o){if(i===a)return;if(typeof i!=typeof a){n.push({path:o,from:i,to:a});return}if(typeof i!="object"||i===null||a===null){i!==a&&n.push({path:o,from:i,to:a});return}if(Array.isArray(i)&&Array.isArray(a)){JSON.stringify(i)!==JSON.stringify(a)&&n.push({path:o,from:i,to:a});return}const r=i,c=a,d=new Set([...Object.keys(r),...Object.keys(c)]);for(const g of d)s(r[g],c[g],o?`${o}.${g}`:g)}return s(e,t,""),n}function kr(e,t=40){let n;try{n=JSON.stringify(e)??String(e)}catch{n=String(e)}return n.length<=t?n:n.slice(0,t-3)+"..."}function Sb(e){const t=e.valid==null?"unknown":e.valid?"valid":"invalid",n=Yl(e.schema),s=n.schema?n.unsupportedPaths.length>0:!1,i=n.schema?.properties??{},a=xr.filter(T=>T.key in i),o=new Set(xr.map(T=>T.key)),r=Object.keys(i).filter(T=>!o.has(T)).map(T=>({key:T,label:T.charAt(0).toUpperCase()+T.slice(1)})),c=[...a,...r],d=e.activeSection&&n.schema&&Ee(n.schema)==="object"?n.schema.properties?.[e.activeSection]:void 0,g=e.activeSection?$b(e.activeSection,d):null,p=e.activeSection?wb({key:e.activeSection,schema:d,uiHints:e.uiHints}):[],h=e.formMode==="form"&&!!e.activeSection&&p.length>0,u=e.activeSubsection===$r,f=e.searchQuery||u?null:e.activeSubsection??p[0]?.key??null,v=e.formMode==="form"?kb(e.originalValue,e.formValue):[],w=e.formMode==="raw"&&e.raw!==e.originalRaw,S=e.formMode==="form"?v.length>0:w,C=!!e.formValue&&!e.loading&&!!n.schema,k=e.connected&&!e.saving&&S&&(e.formMode==="raw"?!0:C),A=e.connected&&!e.applying&&!e.updating&&S&&(e.formMode==="raw"?!0:C),E=e.connected&&!e.applying&&!e.updating;return l`
...
${JSON.stringify(e.issues,null,2)}</pre
  ...
  `}function ey(e,t,n){const s=t.lastUsedAt?ee(t.lastUsedAt):"never",i=t.lastUsedCommand?vi(t.lastUsedCommand,120):null,a=t.lastResolvedPath?vi(t.lastResolvedPath,120):null;return l`
    <div class="list-item">
      <div class="list-main">
      ...
      <div class="card">
        ...
        <div class="form-grid" style="margin-top: 16px;">
          ...
          <label class="field">
            ...
            <select
              .value=${c}
              @change=${d=>{const g=d.target.value;Cn.setLocale(g),e.onSettingsChange({...e.settings,locale:g})}}
            >
  ...
  `}const fy=["","off","minimal","low","medium","high","xhigh"],hy=["","off","on"],my=[{value:"",label:"inherit"},{value:"off",label:"off (explicit)"},{value:"on",label:"on"},{value:"full",label:"full"}],vy=["","off","on","stream"];function by(e){if(!e)return"";const t=e.trim().toLowerCase();return t==="z.ai"||t==="z-ai"?"zai":t}function Cc(e){return by(e)==="zai"}function yy(e){return Cc(e)?hy:fy}function _r(e,t){return t?e.includes(t)?[...e]:[...e,t]:[...e]}function xy(e,t){return t?e.some(n=>n.value===t)?[...e]:[...e,{value:t,label:`${t} (custom)`}]:[...e]}function $y(e,t){return!t||!e||e==="off"?e:"on"}function wy(e,t){return e?t&&e==="on"?"low":e:null}function ky(e){const t=e.result?.sessions??[];return l`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">Sessions</div>
  ...
  `}const Ty=/^data:/i,_y=/^https?:\/\//i;function Ey(e){const t=e.agentsList?.agents??[],s=Wr(e.sessionKey)?.agentId??e.agentsList?.defaultId??"main",a=t.find(r=>r.id===s)?.identity,o=a?.avatarUrl??a?.avatar;if(o)return Ty.test(o)||_y.test(o)?o:a?.avatarUrl}function Ly(e){const t=e.presenceEntries.length,n=e.sessionsResult?.count??null,s=e.cronStatus?.nextWakeAtMs??null,i=e.connected?null:P("chat.disconnected"),a=e.tab==="chat",o=a&&(e.settings.chatFocusMode||e.onboarding),r=e.onboarding?!1:e.settings.chatShowThinking,c=Ey(e),d=e.chatAvatarUrl??c??null,g=e.configForm??e.configSnapshot?.config,p=tn(e.basePath??""),h=e.agentsSelectedId??e.agentsList?.defaultId??e.agentsList?.agents?.[0]?.id??null;return l`
    <div class="shell ${a?"shell--chat":""} ${o?"shell--chat-focus":""} ${e.settings.navCollapsed?"shell--nav-collapsed":""} ${e.onboarding?"shell--onboarding":""}">
      ...
      <main class="content ${a?"content--chat":""}">
        ...
        ${e.tab==="instances"?Fb({loading:e.presenceLoading,entries:e.presenceEntries,lastError:e.presenceError,statusMessage:e.presenceStatus,onRefresh:()=>ca(e)}):m}
