      ...
      <div class="timeseries-chart-wrapper" style="position: relative; cursor: crosshair;">
        ...
        >
          ...
          ${h.map((U,ie)=>{const le=D[ie],fe=_.left+ie*(ce+X),Te=le/F*Q,We=_.top+Q-Te,he=[new Date(U.timestamp).toLocaleDateString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),`${B(le)} tokens`];pe&&(he.push(`Out ${B(U.output)}`),he.push(`In ${B(U.input)}`),he.push(`CW ${B(U.cacheWrite)}`),he.push(`CR ${B(U.cacheRead)}`));const Oe=he.join(" · "),qe=k&&(ie<T||ie>=I);if(!pe)return wt`<rect x="${fe}" y="${We}" width="${ce}" height="${Te}" class="ts-bar${qe?" dimmed":""}" rx="1"><title>${Oe}</title></rect>`;const Ve=[{value:U.output,cls:"output"},{value:U.input,cls:"input"},{value:U.cacheWrite,cls:"cache-write"},{value:U.cacheRead,cls:"cache-read"}];let Ge=_.top+Q;const it=qe?" dimmed":"";return wt`
              ${Ve.map(at=>{if(at.value<=0||le<=0)return m;const yt=Te*(at.value/le);return Ge-=yt,wt`<rect x="${fe}" y="${Ge}" width="${ce}" height="${yt}" class="ts-bar ${at.cls}${it}" rx="1"><title>${Oe}</title></rect>`})}
            `})}
...
`);function nf(e){if(e.loading&&!e.totals)return l`
      ...
      >
        ...
        <div class="usage-filter-popover">
          <div class="usage-filter-actions">
            <button
              class="btn btn-sm"
              @click=${V=>{V.preventDefault(),V.stopPropagation(),e.onQueryDraftChange(Ro(e.queryDraft,M,F))}}
              ?disabled=${X}
            >
              Select All
            </button>
  ...
  `}function sh(e){const t=Is(e.configForm,e.agentId),n=t.entry?.tools??{},s=t.globalTools??{},i=n.profile??s.profile??"full",a=n.profile?"agent override":s.profile?"global default":"default",o=Array.isArray(n.allow)&&n.allow.length>0,r=Array.isArray(s.allow)&&s.allow.length>0,c=!!e.configForm&&!e.configLoading&&!e.configSaving&&!o,d=o?[]:Array.isArray(n.alsoAllow)?n.alsoAllow:[],g=o?[]:Array.isArray(n.deny)?n.deny:[],p=o?{allow:n.allow??[],deny:n.deny??[]}:Wf(i)??void 0,h=zo.flatMap(S=>S.tools.map(C=>C.id)),u=S=>{const C=Kf(S,p),k=Ko(S,d),A=Ko(S,g);return{allowed:(C||k)&&!A,baseAllowed:C,denied:A}},f=h.filter(S=>u(S).allowed).length,v=(S,C)=>{const k=new Set(d.map(I=>Ke(I)).filter(I=>I.length>0)),A=new Set(g.map(I=>Ke(I)).filter(I=>I.length>0)),E=u(S).baseAllowed,T=Ke(S);C?(A.delete(T),E||k.add(T)):(k.delete(T),A.add(T)),e.onOverridesChange(e.agentId,[...k],[...A])},w=S=>{const C=new Set(d.map(A=>Ke(A)).filter(A=>A.length>0)),k=new Set(g.map(A=>Ke(A)).filter(A=>A.length>0));for(const A of h){const E=u(A).baseAllowed,T=Ke(A);S?(k.delete(T),E||C.add(T)):(C.delete(T),k.add(T))}e.onOverridesChange(e.agentId,[...C],[...k])};return l`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
  ...
  `}function dh(e){const{agent:t,configForm:n,agentFilesList:s,agentIdentity:i,agentIdentityLoading:a,agentIdentityError:o,configLoading:r,configSaving:c,configDirty:d,onConfigReload:g,onConfigSave:p,onModelChange:h,onModelFallbacksChange:u}=e,f=Is(n,t.id),w=(s&&s.agentId===t.id?s.workspace:null)||f.entry?.workspace||f.defaults?.workspace||"default",S=f.entry?.model?xn(f.entry?.model):xn(f.defaults?.model),C=xn(f.defaults?.model),k=jo(f.entry?.model)||(S!=="-"?Ho(S):null),A=jo(f.defaults?.model)||(C!=="-"?Ho(C):null),E=k??A??null,T=Bf(f.entry?.model),I=T?T.join(", "):"",H=i?.name?.trim()||t.identity?.name?.trim()||t.name?.trim()||f.entry?.name||"-",te=Ms(t,i)||"-",R=Array.isArray(f.entry?.skills)?f.entry?.skills:null,q=R?.length??null,ne=a?"Loading…":o?"Unavailable":"",oe=!!(e.defaultId&&t.id===e.defaultId);return l`
    <section class="card">
...
`:""}*No output — tool completed successfully.*`;t(p)}:void 0,r=i&&(e.text?.length??0)<=Vv,c=i&&!r,d=i&&r,g=!i;return l`
  ...
  `}function Fa(e,t){const n=Ia(e),s=t?.name?.trim()||"Assistant",i=t?.avatar?.trim()||"",a=n==="user"?"U":n==="assistant"?s.charAt(0).toUpperCase()||"A":n==="tool"?"⚙":"?",o=n==="user"?"user":n==="assistant"?"assistant":n==="tool"?"tool":"other";return i&&n==="assistant"?lb(i)?l`<img
        class="chat-avatar ${o}"
        src="${i}"
        alt="${s}"
  ...
  `;Ds([ws({type:Number})],tn.prototype,"splitRatio",2);Ds([ws({type:Number})],tn.prototype,"minRatio",2);Ds([ws({type:Number})],tn.prototype,"maxRatio",2);tn=Ds([Fr("resizable-divider")],tn);const pb=5e3;function vr(e){e.style.height="auto",e.style.height=`${e.scrollHeight}px`}function fb(e){return e?e.active?l`
