...
`);function nf(e){if(e.loading&&!e.totals)return l`
  ...
  `}function sh(e){const t=Is(e.configForm,e.agentId),n=t.entry?.tools??{},s=t.globalTools??{},i=n.profile??s.profile??"full",a=n.profile?"agent override":s.profile?"global default":"default",o=Array.isArray(n.allow)&&n.allow.length>0,r=Array.isArray(s.allow)&&s.allow.length>0,c=!!e.configForm&&!e.configLoading&&!e.configSaving&&!o,d=o?[]:Array.isArray(n.alsoAllow)?n.alsoAllow:[],g=o?[]:Array.isArray(n.deny)?n.deny:[],p=o?{allow:n.allow??[],deny:n.deny??[]}:Wf(i)??void 0,h=Bo.flatMap(S=>S.tools.map(C=>C.id)),u=S=>{const C=Kf(S,p),k=jo(S,d),A=jo(S,g);return{allowed:(C||k)&&!A,baseAllowed:C,denied:A}},f=h.filter(S=>u(S).allowed).length,v=(S,C)=>{const k=new Set(d.map(I=>je(I)).filter(I=>I.length>0)),A=new Set(g.map(I=>je(I)).filter(I=>I.length>0)),E=u(S).baseAllowed,T=je(S);C?(A.delete(T),E||k.add(T)):(k.delete(T),A.add(T)),e.onOverridesChange(e.agentId,[...k],[...A])},w=S=>{const C=new Set(d.map(A=>je(A)).filter(A=>A.length>0)),k=new Set(g.map(A=>je(A)).filter(A=>A.length>0));for(const A of h){const E=u(A).baseAllowed,T=je(A);S?(k.delete(T),E||C.add(T)):(C.delete(T),k.add(T))}e.onOverridesChange(e.agentId,[...C],[...k])};return l`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
        ...
        <div class="chat-compose__row">
          <label class="field chat-compose__field">
            ...
            <textarea
              ...
              @input=${v=>{const w=v.target;vr(w),e.onDraftChange(w.value)}}
              @paste=${v=>pb(v,e)}
              placeholder=${d}
            ></textarea>
          </label>
...
//# sourceMappingURL=index-DhKry9u_.js.map
