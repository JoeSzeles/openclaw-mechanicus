...
function buildPreferenceContext() {
  ...
  if (summary.insights.length > 0) {
    ctx += "Learned preferences: " + summary.insights.join("; ") + ".\n";
  }
  if (summary.recentPositive.length > 0) {
    ctx += "Recent positive feedback count: " + summary.recentPositive.length + "\n";
  }
  if (summary.recentNegative.length > 0) {
    ctx += "Recent negative feedback count: " + summary.recentNegative.length + "\n";
  }
  ctx += "Adapt your responses based on these learned preferences.\n";
  return ctx;
}
...
async function buildFullPreferenceContext() {
  ...
  if (motorRates && motorRates.motor_rates) {
    const rates = motorRates.motor_rates;
    ctx += "\n[Brain Motor Signals — step " + (motorRates.step_count || 0) + "]\n";
    const sorted = Object.entries(rates).filter(([_, v]) => typeof v === "number" && !isNaN(v)).sort((a, b) => b[1] - a[1]);
    const active = sorted.filter(([_, v]) => v > 0);
    if (active.length > 0) {
      ctx += "Active motor neurons: " + active.map(([k, v]) => k + "=" + v.toFixed(1) + "Hz").join(", ") + "\n";
    } else {
      ctx += "Motor neurons: quiescent (no strong directional signal)\n";
    }
  }
  return ctx;
}
